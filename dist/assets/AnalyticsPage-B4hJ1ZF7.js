import{c as H,d as I,q as j,w as T,g as A,h as U}from"./main-Dih7wz5u.js";import N from"./perf-tobPnqr4.js";class ${static async fetchTransactionsForPeriod(n,e,d){try{const r=H(I,"transactions");try{const t=j(r,T("budgetId","==",n),T("createdAt",">=",e),T("createdAt","<=",d)),x=(await A(t)).docs.map(a=>({id:a.id,...a.data()}));if(x.length>0)return x}catch(t){console.warn("Fallback to broad query (createdAt range unsupported or empty):",t?.message||t)}const s=j(r,T("budgetId","==",n));return(await A(s)).docs.map(t=>({id:t.id,...t.data()})).filter(t=>{const g=$.txToDate(t);return g&&g>=e&&g<=d})}catch(r){return console.error("Erro ao buscar transações do período:",r),[]}}static txToDate(n){try{let e=n?.dataEfetivacao||n?.dataLancamento||n?.data||n?.date||n?.createdAt;if(!e)return null;if(e&&typeof e.toDate=="function")return e.toDate();if(typeof e=="string"){const r=e.trim(),s=r.match(/^(\d{4})-(\d{2})-(\d{2})$/);if(s)return new Date(parseInt(s[1],10),parseInt(s[2],10)-1,parseInt(s[3],10));const c=new Date(r);return isNaN(c.getTime())?null:c}if(typeof e=="number"){const r=e<1e12?e*1e3:e,s=new Date(r);return isNaN(s.getTime())?null:s}if(e instanceof Date)return e;const d=new Date(e);return isNaN(d.getTime())?null:d}catch{return null}}static async getGastosPorCategoria(n,e,d){try{if(console.log("📊 Gerando relatório de gastos por categoria..."),!n)throw new Error("ID do orçamento não fornecido");if(!e||!d){const t=new Date;e=new Date(t.getFullYear(),t.getMonth(),1),d=new Date(t.getFullYear(),t.getMonth()+1,0)}let r=[],s=[];if(window.appState?.transactions&&window.appState?.categories){if(console.log("📊 Usando dados já carregados na aplicação"),r=window.appState.transactions.filter(t=>{if(t.budgetId!==n)return!1;const g=$.txToDate(t);return g&&g>=e&&g<=d}),s=window.appState.categories.filter(t=>t.budgetId===n),!r||r.length===0){const t=await $.fetchTransactionsForPeriod(n,e,d);t.length>0&&(r=t)}}else{console.log("📊 Buscando dados do Firestore..."),r=await $.fetchTransactionsForPeriod(n,e,d);const t=H(I,"categories"),g=j(t,T("budgetId","==",n));s=(await A(g)).docs.map(a=>({id:a.id,...a.data()}))}const c=s.map(t=>{const g=r.filter(a=>a.categoriaId===t.id),x=g.reduce((a,b)=>a+parseFloat(b.valor),0);return{categoria:t,totalGasto:x,transacoes:g,percentual:0}}),p=c.reduce((t,g)=>t+g.totalGasto,0);return c.forEach(t=>{t.percentual=p>0?t.totalGasto/p*100:0}),c.sort((t,g)=>g.totalGasto-t.totalGasto),console.log("✅ Relatório gerado com sucesso:",c),c}catch(r){throw console.error("❌ Erro ao gerar relatório de gastos por categoria:",r),r}}static async getEvolucaoSaldo(n,e=6){try{if(console.log("📊 Gerando relatório de evolução de saldo..."),!n)throw new Error("ID do orçamento não fornecido");const d=new Date,r=[];for(let s=0;s<e;s++){const c=d.getMonth()-s,p=d.getFullYear()+Math.floor(c/12),t=(c%12+12)%12,g=new Date(p,t,1),x=new Date(p,t+1,0);r.push({ano:p,mes:t+1,nome:g.toLocaleString("pt-BR",{month:"long"}),startDate:g,endDate:x,receitas:0,despesas:0,saldo:0})}for(const s of r){let c=[];if(window.appState?.transactions)c=window.appState.transactions.filter(p=>{if(p.budgetId!==n)return!1;const t=$.txToDate(p);return t&&t>=s.startDate&&t<=s.endDate});else{const p=H(I,"transactions"),t=j(p,T("budgetId","==",n),T("createdAt",">=",s.startDate),T("createdAt","<=",s.endDate));c=(await A(t)).docs.map(x=>({id:x.id,...x.data()}))}for(const p of c){const t=parseFloat(p.valor);p.tipo==="receita"?s.receitas+=t:s.despesas+=t}s.saldo=s.receitas-s.despesas}return r.sort((s,c)=>s.ano!==c.ano?c.ano-s.ano:c.mes-s.mes),console.log("✅ Relatório de evolução de saldo gerado com sucesso:",r),r}catch(d){throw console.error("❌ Erro ao gerar relatório de evolução de saldo:",d),d}}static async getPrevisaoGastos(n,e=3,d=3){try{if(console.log("📊 Gerando previsão de gastos..."),!n)throw new Error("ID do orçamento não fornecido");const r=await this.getEvolucaoSaldo(n,e),s=r.reduce((g,x)=>g+x.receitas,0)/r.length,c=r.reduce((g,x)=>g+x.despesas,0)/r.length,p=new Date,t=[];for(let g=1;g<=d;g++){const x=p.getMonth()+g,a=p.getFullYear()+Math.floor(x/12),b=x%12,M=new Date(a,b,1),S=1+g*.01;t.push({ano:a,mes:b+1,nome:M.toLocaleString("pt-BR",{month:"long"}),receitas:s*S,despesas:c*S,saldo:(s-c)*S,isPrevisto:!0})}return console.log("✅ Previsão de gastos gerada com sucesso:",t),t}catch(r){throw console.error("❌ Erro ao gerar previsão de gastos:",r),r}}static renderizarGraficoCategorias(n,e){try{console.log("📊 Renderizando gráfico de categorias...");const d=document.getElementById(n);if(!d)throw new Error(`Elemento com ID ${n} não encontrado`);if(d.innerHTML="",!e||e.length===0){d.innerHTML=`
      ...
      `;d.innerHTML=r,console.log("✅ Gráfico renderizado com sucesso")}catch(d){throw console.error("❌ Erro ao renderizar gráfico de categorias:",d),d}}static renderizarGraficoEvolucao(n,e){try{console.log("📊 Renderizando gráfico de evolução...");const d=document.getElementById(n);if(!d)throw new Error(`Elemento com ID ${n} não encontrado`);if(d.innerHTML="",!e||e.length===0){d.innerHTML=`
        ...
        `;return}const r=Math.max(...e.map(t=>t.receitas)),s=Math.max(...e.map(t=>t.despesas)),c=Math.max(r,s)*1.1,p=`
        <div class="analytics-chart">
      ...
      `;d.innerHTML=p,console.log("✅ Gráfico de evolução renderizado com sucesso")}catch(d){throw console.error("❌ Erro ao renderizar gráfico de evolução:",d),d}}static async gerarRelatorioCompleto(n,e,d){try{if(console.log("📊 Gerando relatório completo..."),!n)throw new Error("ID do orçamento não fornecido");if(!window.appState?.currentUser)throw new Error("Usuário não autenticado");console.log("🔍 Gerando relatório para orçamento:",n);const[r,s,c]=await Promise.all([this.getGastosPorCategoria(n,e,d),this.getEvolucaoSaldo(n,6),this.getPrevisaoGastos(n,3,3)]);console.log("📊 Dados obtidos:",{gastosPorCategoria:r.length,evolucaoSaldo:s.length,previsaoGastos:c.length});let p=0,t=0;if(e&&d){let x=[];window.appState?.transactions&&(x=window.appState.transactions.filter(a=>{if(a.budgetId!==n)return!1;const b=$.txToDate(a);return b&&b>=e&&b<=d})),(!x||x.length===0)&&(x=await $.fetchTransactionsForPeriod(n,e,d)),p=x.filter(a=>a.tipo==="receita").reduce((a,b)=>a+parseFloat(b.valor||0),0),t=x.filter(a=>a.tipo==="despesa").reduce((a,b)=>a+parseFloat(b.valor||0),0)}const g={gastosPorCategoria:r,evolucaoSaldo:s,previsaoGastos:c,resumo:{saldoAtual:p-t,receitasMes:p,despesasMes:t,tendencia:c[0]?.saldo>=0?"positiva":"negativa",categoriasMaioresGastos:r.slice(0,3)}};return console.log("✅ Relatório completo gerado com sucesso"),g}catch(r){throw console.error("❌ Erro ao gerar relatório completo:",r),console.error("Stack trace:",r.stack),r}}}window.Analytics=$;function O(k,{year:n,month:e}={}){try{let D=function(){const l=a?.value||"__all__",i=parseFloat(b?.value||"0")||0,u=parseFloat(M?.value||"")||0;let f=d;if(l!=="__all__"&&(f=f.filter(m=>m.event===l)),f=f.filter(m=>typeof m.durationMs!="number"||m.durationMs>=i),u>0){const m=Date.now()-u*6e4;f=f.filter(y=>(typeof y.ts=="number"?y.ts:0)>=m)}return f},F=function(){const l=D(),i=l.filter(w=>typeof w.durationMs=="number").slice(0,20),u=i.map(w=>w.durationMs||0).sort((w,R)=>w-R),f=i.length?u.reduce((w,R)=>w+R,0)/i.length:0,m=u.length?Math.ceil(.95*u.length)-1:-1,y=m>=0?u[m]:0;t.textContent=String(l.length),g.textContent=`${f.toFixed(1)} ms`,g.className=`text-2xl font-bold ${f>50?"text-red-600":"text-green-600"}`,x&&(x.textContent=`${y.toFixed(1)} ms`,x.className=`text-2xl font-bold ${y>120?"text-red-600":"text-amber-600"}`);const G=l.slice(0,C);p.innerHTML=G.map(w=>{const R=new Date(w.ts).toLocaleTimeString("pt-BR");return`<tr class="border-t border-gray-100 dark:border-gray-800">
        ...
        </tr>`}).join(""),L&&(L.style.display=l.length>C?"":"none")},o=function(l){const i=u=>'"'+String(u??"").replaceAll('"','""')+'"';return["ts,event,durationMs,meta"].concat(l.map(u=>{const{ts:f,event:m,durationMs:y,...G}=u||{},w=Object.keys(G).length?JSON.stringify(G):"";return[f,m,y??""].map(i).concat(i(w)).join(",")})).join(`\r
`)},v=function(){const l=D(),i=o(l);try{window.__lastExportCsv=i}catch{}try{const f=new Blob(["\uFEFF"+i],{type:"text/csv;charset=utf-8;"}),m=URL.createObjectURL(f),y=document.createElement("a");y.href=m,y.download=`telemetria_${n||new Date().getFullYear()}-${String(e||new Date().getMonth()+1).padStart(2,"0")}.csv`,document.body.appendChild(y),y.click(),document.body.removeChild(y),URL.revokeObjectURL(m)}catch{}};if(!(N?.isEnabled&&N.isEnabled()))return;const d=N.getLog&&N.getLog()||[],r=Array.from(new Set(d.map(l=>l.event))).filter(Boolean).sort(),s=document.createElement("div");s.className="mb-8",s.innerHTML=`
    ...
    `,e.appendChild(d);try{const r=n||window?.appState?.currentBudget?.id;if(!r){e.contains(d)&&e.removeChild(d);const o=document.createElement("div");return o.className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-6",o.innerHTML=`
      ...
      `;try{const o=e.querySelector(".tab-header");if(o){const v=document.createElement("button");v.id="analytics-advanced-toggle",v.className="btn btn-outline btn-sm",v.style.marginLeft="auto",v.textContent="Mostrar análises avançadas",o.appendChild(v)}}catch{}if(b){const o=e.querySelector("#analytics-sections")||e,v=document.createElement("div");return v.className="empty-state",v.innerHTML=`
        ...
        `,o.appendChild(v),e}const M=document.createElement("div");M.className="mb-4";const S=a.evolucaoSaldo&&a.evolucaoSaldo.length>1?a.resumo.saldoAtual-a.evolucaoSaldo[1].saldo:null;M.innerHTML=`
        <div class="flex items-center gap-2 mb-4">
        ...
        <div class="bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-gray-800 dark:via-gray-700 dark:to-gray-800 rounded-2xl shadow-lg border border-blue-200 dark:border-gray-600 p-4 mb-6">
          ...
          <div class="flex items-center justify-between mb-4">
            <div>
              ...
              <p class="text-sm text-gray-600 dark:text-gray-400">Período: ${String(t).padStart(2,"0")}/${p}</p>
            </div>
      ...
      `;const P=e.querySelector("#analytics-sections")||e;P.appendChild(M);const E=document.createElement("div");E.id="analytics-advanced";try{const o=typeof localStorage<"u"?localStorage.getItem("analytics.advancedVisible"):null;(!o||o!=="1")&&E.classList.add("hidden")}catch{E.classList.add("hidden")}P.appendChild(E);try{const o=i=>{const u=parseFloat(String(i??"").toString().replace(",","."));return isNaN(u)?0:u};let v=0;const h=window?.appState?.currentBudget||null;h&&o(h.valorTotal)>0?v=o(h.valorTotal):Array.isArray(window?.appState?.categories)&&(v=(window.appState.categories||[]).filter(i=>i&&(i.tipo||"despesa")==="despesa").reduce((i,u)=>i+(o(u.limite)>0?o(u.limite):0),0));const l=o(a?.resumo?.despesasMes||0);if(v>0&&l>=0){const i=Math.max(0,Math.min(100,l/v*100)),u=new Date,f=p===u.getFullYear()&&t===u.getMonth()+1,m=new Date(p,t,0).getDate(),y=f?u.getDate():m,G=f?Math.min(100,y/m*100):100,w=v*G/100,R=w>0?l/w:0,z=R<=1.05?"ok":R<=1.2?"warn":"bad",q=96,B=42,Y=2*Math.PI*B,V=i/100*Y,_=document.createElement("div");_.className="mb-8",_.innerHTML=`
            <div class="flex items-center gap-2 mb-4">
          ...
          `,P.appendChild(_)}}catch(o){console.warn("MTD progress render skipped:",o?.message||o)}const L=document.createElement("div");L.className="mb-8",L.innerHTML=`
        <div class="flex items-center gap-2 mb-4">
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          <div class="p-4">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              ${a.previsaoGastos.slice(0,4).map((o,v)=>`
      ...
      `,E.appendChild(C),O(E,{year:p,month:t});try{const v=(await $.fetchTransactionsForPeriod(s,g,x)||[]).filter(m=>(m?.tipo||"despesa")==="despesa"),h=Array.isArray(window?.appState?.categories)?window.appState.categories:[],l=new Map(h.map(m=>[m.id,m])),i=m=>{try{return new Date(m).toLocaleDateString("pt-BR")}catch{return""}},u=v.map(m=>({...m,_date:$.txToDate(m),_cat:l.get(m.categoriaId)||m.categoria||null,_valor:parseFloat(m.valor||0)}));u.sort((m,y)=>(y._valor||0)-(m._valor||0));const f=u.slice(0,5);if(f.length){const m=document.createElement("div");m.className="mb-8",m.innerHTML=`
            <div class="flex items-center gap-2 mb-4">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <div class="p-4">
              <div class="space-y-4">
                ${a.gastosPorCategoria.slice(0,5).map((o,v)=>`
      ...
      `,E.appendChild(D);const F=(o=1,v=5)=>{const h=document.getElementById("evolucao-chart"),l=document.getElementById("categorias-chart");if(console.log(`🔍 Tentativa ${o}: Verificando elementos dos gráficos...`),console.log("- evolucao-chart:",!!h),console.log("- categorias-chart:",!!l),h&&l)try{if(console.log("📊 Renderizando gráficos..."),a.evolucaoSaldo&&a.evolucaoSaldo.length>0){const i=[...a.evolucaoSaldo];a.previsaoGastos&&a.previsaoGastos.length>0&&i.push(...a.previsaoGastos),$.renderizarGraficoEvolucao("evolucao-chart",i)}a.gastosPorCategoria&&a.gastosPorCategoria.length>0&&$.renderizarGraficoCategorias("categorias-chart",a.gastosPorCategoria),console.log("✅ Gráficos renderizados com sucesso!")}catch(i){console.error("❌ Erro ao renderizar gráficos:",i),h&&(h.innerHTML=`
              ...
              `)}else if(o<v)console.log(`⏳ Elementos não encontrados, tentando novamente em ${o*100}ms...`),setTimeout(()=>F(o+1,v),o*100);else{console.error("❌ Não foi possível encontrar os elementos dos gráficos após",v,"tentativas");const i=document.getElementById("evolucao-chart"),u=document.getElementById("categorias-chart");i&&(i.innerHTML=`
              <div class="text-center py-8">
            ...
            `)}};setTimeout(()=>F(),100);try{const o=e.querySelector("#analytics-advanced-toggle"),v=e.querySelector("#analytics-advanced");if(o&&v){const h=()=>{const l=v.classList.contains("hidden");o.textContent=l?"Mostrar análises avançadas":"Ocultar análises avançadas"};h(),o.addEventListener("click",()=>{v.classList.toggle("hidden"),h();try{localStorage.setItem("analytics.advancedVisible",v.classList.contains("hidden")?"0":"1")}catch{}})}}catch{}}catch(r){console.error("❌ Erro ao renderizar página de análises:",r),e.contains(d)&&e.removeChild(d);const s=document.createElement("div");s.className="bg-red-100 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 px-4 py-3 rounded relative mb-6",s.innerHTML=`

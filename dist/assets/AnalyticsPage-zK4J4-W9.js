import{c as H,d as I,q as j,w as P,g as A,h as Y}from"./main-tWUdnkzv.js";import N from"./perf-tobPnqr4.js";class ${static async fetchTransactionsForPeriod(n,e,d){try{const r=H(I,"transactions");try{const t=j(r,P("budgetId","==",n),P("createdAt",">=",e),P("createdAt","<=",d)),x=(await A(t)).docs.map(a=>({id:a.id,...a.data()}));if(x.length>0)return x}catch(t){console.warn("Fallback to broad query (createdAt range unsupported or empty):",t?.message||t)}const o=j(r,P("budgetId","==",n));return(await A(o)).docs.map(t=>({id:t.id,...t.data()})).filter(t=>{const g=$.txToDate(t);return g&&g>=e&&g<=d})}catch(r){return console.error("Erro ao buscar transações do período:",r),[]}}static txToDate(n){try{let e=n?.dataEfetivacao||n?.dataLancamento||n?.data||n?.date||n?.createdAt;if(!e)return null;if(e&&typeof e.toDate=="function")return e.toDate();if(typeof e=="string"){const r=e.trim(),o=r.match(/^(\d{4})-(\d{2})-(\d{2})$/);if(o)return new Date(parseInt(o[1],10),parseInt(o[2],10)-1,parseInt(o[3],10));const c=new Date(r);return isNaN(c.getTime())?null:c}if(typeof e=="number"){const r=e<1e12?e*1e3:e,o=new Date(r);return isNaN(o.getTime())?null:o}if(e instanceof Date)return e;const d=new Date(e);return isNaN(d.getTime())?null:d}catch{return null}}static async getGastosPorCategoria(n,e,d){try{if(console.log("📊 Gerando relatório de gastos por categoria..."),!n)throw new Error("ID do orçamento não fornecido");if(!e||!d){const t=new Date;e=new Date(t.getFullYear(),t.getMonth(),1),d=new Date(t.getFullYear(),t.getMonth()+1,0)}let r=[],o=[];if(window.appState?.transactions&&window.appState?.categories){if(console.log("📊 Usando dados já carregados na aplicação"),r=window.appState.transactions.filter(t=>{if(t.budgetId!==n)return!1;const g=$.txToDate(t);return g&&g>=e&&g<=d}),o=window.appState.categories.filter(t=>t.budgetId===n),!r||r.length===0){const t=await $.fetchTransactionsForPeriod(n,e,d);t.length>0&&(r=t)}}else{console.log("📊 Buscando dados do Firestore..."),r=await $.fetchTransactionsForPeriod(n,e,d);const t=H(I,"categories"),g=j(t,P("budgetId","==",n));o=(await A(g)).docs.map(a=>({id:a.id,...a.data()}))}const c=o.map(t=>{const g=r.filter(a=>a.categoriaId===t.id),x=g.reduce((a,b)=>a+parseFloat(b.valor),0);return{categoria:t,totalGasto:x,transacoes:g,percentual:0}}),p=c.reduce((t,g)=>t+g.totalGasto,0);return c.forEach(t=>{t.percentual=p>0?t.totalGasto/p*100:0}),c.sort((t,g)=>g.totalGasto-t.totalGasto),console.log("✅ Relatório gerado com sucesso:",c),c}catch(r){throw console.error("❌ Erro ao gerar relatório de gastos por categoria:",r),r}}static async getEvolucaoSaldo(n,e=6){try{if(console.log("📊 Gerando relatório de evolução de saldo..."),!n)throw new Error("ID do orçamento não fornecido");const d=new Date,r=[];for(let o=0;o<e;o++){const c=d.getMonth()-o,p=d.getFullYear()+Math.floor(c/12),t=(c%12+12)%12,g=new Date(p,t,1),x=new Date(p,t+1,0);r.push({ano:p,mes:t+1,nome:g.toLocaleString("pt-BR",{month:"long"}),startDate:g,endDate:x,receitas:0,despesas:0,saldo:0})}for(const o of r){let c=[];if(window.appState?.transactions)c=window.appState.transactions.filter(p=>{if(p.budgetId!==n)return!1;const t=$.txToDate(p);return t&&t>=o.startDate&&t<=o.endDate});else{const p=H(I,"transactions"),t=j(p,P("budgetId","==",n),P("createdAt",">=",o.startDate),P("createdAt","<=",o.endDate));c=(await A(t)).docs.map(x=>({id:x.id,...x.data()}))}for(const p of c){const t=parseFloat(p.valor);p.tipo==="receita"?o.receitas+=t:o.despesas+=t}o.saldo=o.receitas-o.despesas}return r.sort((o,c)=>o.ano!==c.ano?c.ano-o.ano:c.mes-o.mes),console.log("✅ Relatório de evolução de saldo gerado com sucesso:",r),r}catch(d){throw console.error("❌ Erro ao gerar relatório de evolução de saldo:",d),d}}static async getPrevisaoGastos(n,e=3,d=3){try{if(console.log("📊 Gerando previsão de gastos..."),!n)throw new Error("ID do orçamento não fornecido");const r=await this.getEvolucaoSaldo(n,e),o=r.reduce((g,x)=>g+x.receitas,0)/r.length,c=r.reduce((g,x)=>g+x.despesas,0)/r.length,p=new Date,t=[];for(let g=1;g<=d;g++){const x=p.getMonth()+g,a=p.getFullYear()+Math.floor(x/12),b=x%12,M=new Date(a,b,1),S=1+g*.01;t.push({ano:a,mes:b+1,nome:M.toLocaleString("pt-BR",{month:"long"}),receitas:o*S,despesas:c*S,saldo:(o-c)*S,isPrevisto:!0})}return console.log("✅ Previsão de gastos gerada com sucesso:",t),t}catch(r){throw console.error("❌ Erro ao gerar previsão de gastos:",r),r}}static renderizarGraficoCategorias(n,e){try{console.log("📊 Renderizando gráfico de categorias...");const d=document.getElementById(n);if(!d)throw new Error(`Elemento com ID ${n} não encontrado`);if(d.innerHTML="",!e||e.length===0){d.innerHTML=`
      ...
      `;d.innerHTML=r,console.log("✅ Gráfico renderizado com sucesso")}catch(d){throw console.error("❌ Erro ao renderizar gráfico de categorias:",d),d}}static renderizarGraficoEvolucao(n,e){try{console.log("📊 Renderizando gráfico de evolução...");const d=document.getElementById(n);if(!d)throw new Error(`Elemento com ID ${n} não encontrado`);if(d.innerHTML="",!e||e.length===0){d.innerHTML=`
        ...
        `;return}const r=Math.max(...e.map(t=>t.receitas)),o=Math.max(...e.map(t=>t.despesas)),c=Math.max(r,o)*1.1,p=`
        <div class="analytics-chart">
      ...
      `;d.innerHTML=p,console.log("✅ Gráfico de evolução renderizado com sucesso")}catch(d){throw console.error("❌ Erro ao renderizar gráfico de evolução:",d),d}}static async gerarRelatorioCompleto(n,e,d){try{if(console.log("📊 Gerando relatório completo..."),!n)throw new Error("ID do orçamento não fornecido");if(!window.appState?.currentUser)throw new Error("Usuário não autenticado");console.log("🔍 Gerando relatório para orçamento:",n);const[r,o,c]=await Promise.all([this.getGastosPorCategoria(n,e,d),this.getEvolucaoSaldo(n,6),this.getPrevisaoGastos(n,3,3)]);console.log("📊 Dados obtidos:",{gastosPorCategoria:r.length,evolucaoSaldo:o.length,previsaoGastos:c.length});let p=0,t=0;if(e&&d){let x=[];window.appState?.transactions&&(x=window.appState.transactions.filter(a=>{if(a.budgetId!==n)return!1;const b=$.txToDate(a);return b&&b>=e&&b<=d})),(!x||x.length===0)&&(x=await $.fetchTransactionsForPeriod(n,e,d)),p=x.filter(a=>a.tipo==="receita").reduce((a,b)=>a+parseFloat(b.valor||0),0),t=x.filter(a=>a.tipo==="despesa").reduce((a,b)=>a+parseFloat(b.valor||0),0)}const g={gastosPorCategoria:r,evolucaoSaldo:o,previsaoGastos:c,resumo:{saldoAtual:p-t,receitasMes:p,despesasMes:t,tendencia:c[0]?.saldo>=0?"positiva":"negativa",categoriasMaioresGastos:r.slice(0,3)}};return console.log("✅ Relatório completo gerado com sucesso"),g}catch(r){throw console.error("❌ Erro ao gerar relatório completo:",r),console.error("Stack trace:",r.stack),r}}}window.Analytics=$;function O(k,{year:n,month:e}={}){try{let D=function(){const l=a?.value||"__all__",i=parseFloat(b?.value||"0")||0,u=parseFloat(M?.value||"")||0;let f=d;if(l!=="__all__"&&(f=f.filter(m=>m.event===l)),f=f.filter(m=>typeof m.durationMs!="number"||m.durationMs>=i),u>0){const m=Date.now()-u*6e4;f=f.filter(y=>(typeof y.ts=="number"?y.ts:0)>=m)}return f},F=function(){const l=D(),i=l.filter(w=>typeof w.durationMs=="number").slice(0,20),u=i.map(w=>w.durationMs||0).sort((w,T)=>w-T),f=i.length?u.reduce((w,T)=>w+T,0)/i.length:0,m=u.length?Math.ceil(.95*u.length)-1:-1,y=m>=0?u[m]:0;t.textContent=String(l.length),g.textContent=`${f.toFixed(1)} ms`,g.className=`text-2xl font-bold ${f>50?"text-red-600":"text-green-600"}`,x&&(x.textContent=`${y.toFixed(1)} ms`,x.className=`text-2xl font-bold ${y>120?"text-red-600":"text-amber-600"}`);const G=l.slice(0,C);p.innerHTML=G.map(w=>{const T=new Date(w.ts).toLocaleTimeString("pt-BR");return`<tr class="border-t border-gray-100 dark:border-gray-800">
        ...
        </tr>`}).join(""),L&&(L.style.display=l.length>C?"":"none")},s=function(l){const i=u=>'"'+String(u??"").replaceAll('"','""')+'"';return["ts,event,durationMs,meta"].concat(l.map(u=>{const{ts:f,event:m,durationMs:y,...G}=u||{},w=Object.keys(G).length?JSON.stringify(G):"";return[f,m,y??""].map(i).concat(i(w)).join(",")})).join(`\r
`)},v=function(){const l=D(),i=s(l);try{window.__lastExportCsv=i}catch{}try{const f=new Blob(["\uFEFF"+i],{type:"text/csv;charset=utf-8;"}),m=URL.createObjectURL(f),y=document.createElement("a");y.href=m,y.download=`telemetria_${n||new Date().getFullYear()}-${String(e||new Date().getMonth()+1).padStart(2,"0")}.csv`,document.body.appendChild(y),y.click(),document.body.removeChild(y),URL.revokeObjectURL(m)}catch{}};if(!(N?.isEnabled&&N.isEnabled()))return;const d=N.getLog&&N.getLog()||[],r=Array.from(new Set(d.map(l=>l.event))).filter(Boolean).sort(),o=document.createElement("div");o.className="mb-8",o.innerHTML=`
    ...
    `,e.appendChild(d);try{const r=n||window?.appState?.currentBudget?.id;if(!r){e.contains(d)&&e.removeChild(d);const s=document.createElement("div");return s.className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-6",s.innerHTML=`
      ...
      <div class="flex items-center justify-between mb-6">
        ...
        <p class="text-xs opacity-90 mt-1">Período: ${String(t).padStart(2,"0")}/${p}</p>
            </div>
            <div class="text-right">
              <div class="text-2xl md:text-3xl font-bold ${a.resumo.saldoAtual>=0?"text-green-200":"text-red-200"}">
          ...
          <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
            <div class="bg-white bg-opacity-15 backdrop-blur-sm rounded-xl p-4 text-center">
              ...
              <div class="text-2xl md:text-3xl font-bold text-green-200">R$ ${a.resumo.receitasMes.toFixed(2)}</div>
              <div class="text-sm opacity-90">Receitas</div>
            </div>
            
            <div class="bg-white bg-opacity-15 backdrop-blur-sm rounded-xl p-4 text-center">
              <div class="text-2xl mb-2">📉</div>
              <div class="text-2xl md:text-3xl font-bold text-red-200">R$ ${a.resumo.despesasMes.toFixed(2)}</div>
              <div class="text-sm opacity-90">Despesas</div>
            </div>
            
            <div class="bg-white bg-opacity-15 backdrop-blur-sm rounded-xl p-4 text-center">
              <div class="text-2xl mb-2">📊</div>
              <div class="text-2xl md:text-3xl font-bold ${a.resumo.tendencia==="positiva"?"text-green-200":"text-red-200"}">
          ...
          ${S!==null?`
            <div class="mt-4 text-xs opacity-90">
              Variação vs mês anterior: <span class="font-semibold ${S>=0?"text-green-200":"text-red-200"}">${S>=0?"+":""}R$ ${Math.abs(S).toFixed(2)}</span>
            </div>
          `:""}
        </div>
      `;const R=e.querySelector("#analytics-sections")||e;R.appendChild(M);const E=document.createElement("div");E.id="analytics-advanced";try{const s=typeof localStorage<"u"?localStorage.getItem("analytics.advancedVisible"):null;(!s||s!=="1")&&E.classList.add("hidden")}catch{E.classList.add("hidden")}R.appendChild(E);try{const s=i=>{const u=parseFloat(String(i??"").toString().replace(",","."));return isNaN(u)?0:u};let v=0;const h=window?.appState?.currentBudget||null;h&&s(h.valorTotal)>0?v=s(h.valorTotal):Array.isArray(window?.appState?.categories)&&(v=(window.appState.categories||[]).filter(i=>i&&(i.tipo||"despesa")==="despesa").reduce((i,u)=>i+(s(u.limite)>0?s(u.limite):0),0));const l=s(a?.resumo?.despesasMes||0);if(v>0&&l>=0){const i=Math.max(0,Math.min(100,l/v*100)),u=new Date,f=p===u.getFullYear()&&t===u.getMonth()+1,m=new Date(p,t,0).getDate(),y=f?u.getDate():m,G=f?Math.min(100,y/m*100):100,w=v*G/100,T=w>0?l/w:0,z=T<=1.05?"ok":T<=1.2?"warn":"bad",q=96,B=42,V=2*Math.PI*B,U=i/100*V,_=document.createElement("div");_.className="mb-8",_.innerHTML=`
            <div class="flex items-center gap-2 mb-4">
          ...
          `,R.appendChild(_)}}catch(s){console.warn("MTD progress render skipped:",s?.message||s)}const L=document.createElement("div");L.className="mb-8",L.innerHTML=`
        <div class="flex items-center gap-2 mb-4">
      ...
      `,E.appendChild(C),O(E,{year:p,month:t});try{const v=(await $.fetchTransactionsForPeriod(o,g,x)||[]).filter(m=>(m?.tipo||"despesa")==="despesa"),h=Array.isArray(window?.appState?.categories)?window.appState.categories:[],l=new Map(h.map(m=>[m.id,m])),i=m=>{try{return new Date(m).toLocaleDateString("pt-BR")}catch{return""}},u=v.map(m=>({...m,_date:$.txToDate(m),_cat:l.get(m.categoriaId)||m.categoria||null,_valor:parseFloat(m.valor||0)}));u.sort((m,y)=>(y._valor||0)-(m._valor||0));const f=u.slice(0,5);if(f.length){const m=document.createElement("div");m.className="mb-8",m.innerHTML=`
            <div class="flex items-center gap-2 mb-4">
          ...
          `,R.appendChild(l)}}catch{}const D=document.createElement("div");D.className="mb-8",D.innerHTML=`
        <div class="flex items-center gap-2 mb-4">

              ...
              <div class="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div class="flex items-center gap-1">
                    <span class="text-cyan-600 dark:text-cyan-400 text-xs">${p.length} categorias</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-gray-800 dark:via-gray-700 dark:to-gray-800 rounded-2xl shadow-lg border border-blue-200 dark:border-gray-600 p-4 mb-6">
              ...
              <div class="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p class="text-sm text-gray-600 dark:text-gray-400">${w} categorias • ${String(_).padStart(2,"0")}/${I}</p>
                </div>
              ...
              <div class="grid grid-cols-4 gap-3 mb-4">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg p-3 text-center shadow-sm border border-gray-200 dark:border-gray-600">
                  <div class="text-lg mb-1">💸</div>
                  <div class="text-lg font-bold text-red-600 dark:text-red-400">${m}</div>
...
${p.length===0?`
                    ...
                    <div class="u-card bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-2xl transition-all duration-300 group ${k?"ring-2 ring-red-200 dark:ring-red-800":""}">
                      ...
                      <div class="bg-gradient-to-r ${s?"from-green-50 to-emerald-50 dark:from-gray-800 dark:to-gray-800":"from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-800"} p-4 border-b border-gray-200 dark:border-gray-700">
                        <div class="flex items-center justify-between">
                          ...
                          ${k?'<div class="text-2xl">🚨</div>':y&&t.porcentagem>=90?'<div class="text-2xl">⚠️</div>':""}
                        </div>
                  </div>
                        ...
                        ${y?`
                          ...
                          <div class="space-y-2 mb-4">
                            ...
                            <div class="text-xs text-gray-500 dark:text-gray-400">
                              ${k?`🚨 Excedeu em R$ ${Math.abs(t.saldo).toFixed(2)}`:`Saldo: R$ ${t.saldo.toFixed(2)}`}
                            </div>
                          </div>
                        `:`
                          <div class="space-y-2 mb-4">
  ...
  `;try{const t=document.querySelector(".content-spacing");if(t&&w===0){const s=document.createElement("div");s.className="empty-state mt-2",s.innerHTML='<div class="empty-icon">📁</div><div class="empty-text">Nenhuma categoria cadastrada</div><div class="mt-2"><button class="u-btn u-btn--primary mobile-btn w-full" onclick="window.showAddCategoryModal && window.showAddCategoryModal()">Criar categoria</button></div>',t.appendChild(s)}}catch{}setTimeout(()=>{try{q()}catch(t){console.warn("setupCategoryButtons falhou:",t)}try{const t=document.getElementById("add-category-btn");t&&(t.onclick=()=>window.showAddCategoryModal?.())}catch{}},100),J();try{window.__catPeriodListenerBound||(window.__catPeriodListenerBound=!0,B.on("period:changed",t=>queueMicrotask(()=>{const s=(window.location.hash||"").split("?")[0];if(s==="#/categories"){try{const y=t&&t.year||G().year,k=t&&t.month||G().month,C=`${y}-${String(k).padStart(2,"0")}`,b=new URL(window.location.href);b.hash=`${s}?ym=${C}`,history.replaceState(null,"",b.toString())}catch{}M()}})))}catch{}z();try{typeof window.deleteCategoryWithConfirmation!="function"&&(window.deleteCategoryWithConfirmation=async function(t,s="categoria"){try{if(!confirm(`Excluir "${s||"categoria"}"? Esta ação não pode ser desfeita.`))return;await(await O(()=>import("./main-D7uShLQF.js").then(b=>b.a2),__vite__mapDeps([0,1]))).deleteCategory(t);try{window.Snackbar?.show?.("Categoria excluída","success")}catch{}try{await M()}catch{}}catch(y){console.error("Falha ao excluir categoria:",y);try{window.Snackbar?.show?.("Erro ao excluir categoria","error")}catch{}}}),typeof window.editCategory!="function"&&(window.editCategory=function(t){try{const s=window.appState?.categories?.find(y=>y.id===t);s&&window.showAddCategoryModal?.(s)}catch(s){console.warn("editCategory (fallback) falhou:",s)}}),typeof window.showCategoryHistory!="function"&&(window.showCategoryHistory=function(t){try{const{year:s,month:y}=G(),C=`Histórico - ${window.appState?.categories?.find(o=>o.id===t)?.nome||t}`,b=o=>{if(!o)return null;if(typeof o?.toDate=="function")return o.toDate();if(typeof o=="object"&&o?.seconds)return new Date(o.seconds*1e3);const x=new Date(o);return isNaN(x.getTime())?null:x},R=o=>(Number.parseFloat(o||0)||0).toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}),u=`<div class="max-h-96 overflow-auto">${(window.appState?.transactions||[]).filter(o=>o.categoriaId===t).filter(o=>{const x=b(o.createdAt);return x?x.getFullYear()===s&&x.getMonth()+1===y:!1}).sort((o,x)=>{const A=b(o.createdAt)||0;return(b(x.createdAt)||0)-A}).slice(0,100).map(o=>{const x=b(o.createdAt),A=x?x.toLocaleDateString("pt-BR"):"-",L=o.tipo==="receita"?"+":"-",N=o.descricao&&String(o.descricao).trim()||"(Sem descrição)",P=o.tipo==="receita"?"text-green-600":"text-red-600";return`

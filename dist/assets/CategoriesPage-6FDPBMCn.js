const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/showAddCategoryModal-B8Qdi_2f.js","assets/main-C6mVWVKS.js","assets/main-CF1rWTKC.css"])))=>i.map(i=>d[i]);
import{h as E,e as B,_ as j,i as U}from"./main-C6mVWVKS.js";import{mountPeriodIndicator as _}from"./PeriodIndicator-CK1vxorZ.js";import{a as W,r as V}from"./UIService-Bhth4ObY.js";typeof window<"u"&&window.eventBus&&typeof window.eventBus.on=="function"&&window.eventBus.on("categories:updated",()=>{if((window.location.hash||"").split("?")[0]==="#/categories")try{T()}catch(i){console.warn("Falha ao renderizar categorias após atualização",i)}});async function K(s){const i=document.createElement("div");if(i.className="categories-page",!window.showAddCategoryModal)try{const{default:o}=await j(async()=>{const{default:x}=await import("./showAddCategoryModal-B8Qdi_2f.js");return{default:x}},__vite__mapDeps([0,1,2]));window.showAddCategoryModal=o,console.log("✅ showAddCategoryModal carregado dinamicamente")}catch(o){console.error("❌ Falha ao carregar showAddCategoryModal:",o)}const f=document.createElement("div");f.className="tab-header mb-6",f.innerHTML=`
...
${o.length===0?`
            ...
            `:`
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ${u.map(e=>{const l=e.tipo==="receita",g=e.limite>0,$=g&&e.totalGasto>e.limite;return`
                    <div class="u-card bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-2xl transition-all duration-300 group ${$?"ring-2 ring-red-200 dark:ring-red-800":""}" data-category-id="${e.id}">
                      <!-- Header da Categoria -->
                      <div class="bg-gradient-to-r ${l?"from-green-50 to-emerald-50 dark:from-gray-800 dark:to-gray-800":"from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-800"} p-4 border-b border-gray-200 dark:border-gray-700">
                        <div class="flex items-center justify-between">
                          <div class="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p class="text-sm text-gray-500 dark:text-gray-400">${l?"Receita":"Despesa"} • ${String(d).padStart(2,"0")}/${a}</p>
                            </div>
                          </div>
                          ${$?'<div class="text-2xl">🚨</div>':g&&e.porcentagem>=90?'<div class="text-2xl">⚠️</div>':""}
                        </div>
                  </div>
  ...
  `;try{const e=document.querySelector(".content-spacing");if(e&&x===0){const l=document.createElement("div");l.className="empty-state mt-2",l.innerHTML='<div class="empty-icon">📁</div><div class="empty-text">Nenhuma categoria cadastrada</div><div class="mt-2"><button class="btn btn-primary w-full" onclick="window.showAddCategoryModal && window.showAddCategoryModal()">Criar categoria</button></div>',e.appendChild(l)}}catch{}setTimeout(()=>{try{W()}catch(e){console.warn("setupCategoryButtons falhou:",e)}try{const e=document.getElementById("add-category-btn");e&&(e.onclick=()=>window.showAddCategoryModal?.())}catch{}},100),ae();try{window.__catPeriodListenerBound||(window.__catPeriodListenerBound=!0,B.on("period:changed",e=>queueMicrotask(()=>{const l=(window.location.hash||"").split("?")[0];if(l==="#/categories"){try{const g=e&&e.year||E().year,$=e&&e.month||E().month,R=`${g}-${String($).padStart(2,"0")}`,b=new URL(window.location.href);b.hash=`${l}?ym=${R}`,history.replaceState(null,"",b.toString())}catch{}console.log("🔄 Período mudou, verificando se deve re-renderizar...");try{const g=document.getElementById("app-content");if(console.log("🔍 Conteúdo da página atual:",g?g.innerHTML.substring(0,200)+"...":"null"),g&&(g.innerHTML.includes("📂 Categorias")||g.innerHTML.includes("cat-period-indicator")||g.innerHTML.includes("categories-page")||g.querySelector("#cat-period-indicator")!==null)){console.log("✅ Página de categorias já carregada, ATUALIZANDO dados do período"),console.log("🎯 Atualizando apenas os dados sem re-renderizar header"),J();return}else console.log("⚠️ Página de categorias não encontrada, re-renderizando...")}catch(g){console.warn("Erro ao verificar página atual:",g)}console.log("🚀 Chamando renderCategories()..."),T()}})))}catch{}V();try{typeof window.deleteCategoryWithConfirmation!="function"&&(window.deleteCategoryWithConfirmation=async function(e,l="categoria"){try{const g=l||"categoria";let $=!1;if(typeof window.confirmDelete=="function"?$=await window.confirmDelete(g,"categoria"):typeof window.showConfirmationModal=="function"?$=await new Promise(b=>{window.showConfirmationModal({title:"Excluir Categoria",message:`Tem certeza que deseja excluir "${g}"? Esta ação não pode ser desfeita.`,confirmText:"Sim, excluir",confirmColor:"bg-red-500 hover:bg-red-600",onConfirm:()=>b(!0),onCancel:()=>b(!1)})}):$=confirm(`Excluir "${g}"? Esta ação não pode ser desfeita.`),!$)return;await(await j(()=>import("./main-C6mVWVKS.js").then(b=>b.a2),__vite__mapDeps([1,2]))).deleteCategory(e);try{window.Snackbar?.show?.("Categoria excluída","success")}catch{}try{await T()}catch{}}catch(g){console.error("Falha ao excluir categoria:",g);try{window.Snackbar?.show?.("Erro ao excluir categoria","error")}catch{}}}),typeof window.editCategory!="function"&&(window.editCategory=function(e){console.log("🔧 editCategory chamada com ID:",e);try{const l=window.appState?.categories?.find(g=>g.id===e);console.log("🔧 Categoria encontrada:",l),console.log("🔧 window.showAddCategoryModal disponível:",typeof window.showAddCategoryModal),l?window.showAddCategoryModal?(console.log("✅ Chamando showAddCategoryModal"),window.showAddCategoryModal(l)):(console.log("🔄 showAddCategoryModal não disponível, tentando carregar..."),j(()=>import("./showAddCategoryModal-B8Qdi_2f.js"),__vite__mapDeps([0,1,2])).then(g=>{console.log("✅ Modal carregado dinamicamente"),g.default&&(window.showAddCategoryModal=g.default,g.default(l))}).catch(g=>{console.error("❌ Falha ao carregar modal:",g),window.Snackbar&&window.Snackbar.show("Erro ao abrir modal de edição","error")})):(console.error("❌ Categoria não encontrada:",e),window.Snackbar&&window.Snackbar.show("Categoria não encontrada","error"))}catch(l){console.warn("editCategory (fallback) falhou:",l),window.Snackbar&&window.Snackbar.show("Erro ao editar categoria","error")}}),typeof window.showCategoryHistory!="function"&&(window.showCategoryHistory=function(e){try{const{year:l,month:g}=E(),R=`Histórico - ${window.appState?.categories?.find(m=>m.id===e)?.nome||e}`,b=m=>{if(!m)return null;if(typeof m?.toDate=="function")return m.toDate();if(typeof m=="object"&&m?.seconds)return new Date(m.seconds*1e3);const h=new Date(m);return isNaN(h.getTime())?null:h},S=m=>(Number.parseFloat(m||0)||0).toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}),v=`<div class="max-h-96 overflow-auto">${(window.appState?.transactions||[]).filter(m=>m.categoriaId===e).filter(m=>{const h=b(m.createdAt);return h?h.getFullYear()===l&&h.getMonth()+1===g:!1}).sort((m,h)=>{const D=b(m.createdAt)||0;return(b(h.createdAt)||0)-D}).slice(0,100).map(m=>{const h=b(m.createdAt),D=h?h.toLocaleDateString("pt-BR"):"-",I=m.tipo==="receita"?"+":"-",O=m.descricao&&String(m.descricao).trim()||"(Sem descrição)",Y=m.tipo==="receita"?"text-green-600":"text-red-600";return`

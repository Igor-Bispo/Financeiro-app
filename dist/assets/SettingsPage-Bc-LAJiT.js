...
import{_ as v,e as c}from"./main-kf7gv-aY.js";import{loadBudgetInvitations as u,loadSentBudgetInvitations as y,fetchUsersInfo as f,fetchUserInfo as h}from"./settings.service-C39IPhGs.js";function b(s){s?(document.body.classList.remove("no-animations"),localStorage.setItem("animationsEnabled","1")):(document.body.classList.add("no-animations"),localStorage.setItem("animationsEnabled","0"))}document.addEventListener("DOMContentLoaded",()=>{const s=localStorage.getItem("animationsEnabled"),e=s===null?!0:s==="1";b(e);const a=document.getElementById("animations-toggle");a&&(a.checked=e,a.addEventListener("change",r=>{b(r.target.checked)}))});function x(s,e,a){return`
    ...
    <div class="space-y-3">
        ...
        `).join("")}
    </div>`}function $(s){return!s||s.length===0?`
        <div class="text-center py-8">
            <div class="text-4xl mb-3">📬</div>
            <div class="text-gray-500 dark:text-gray-400">Nenhum convite pendente</div>
    ...
    <div class="space-y-3">
        ...
        `).join("")}
    </div>`}function E(s){const{currentUser:e,currentBudget:a,budgets:r,usersWithAccess:d,ownerDisplay:t,pendingInvitations:n,sentInvitations:l,appVersion:i}=s;return`
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        ...
                                        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                                            ...
                                            <div class="font-bold text-sm text-gray-800 dark:text-gray-200">v4.38.0</div>
                                        </div>
                                    </div>
    ...
    `}async function A(){const s=document.getElementById("app-content");if(!s){console.warn("⚠️ SettingsPage: elemento #app-content não encontrado");return}try{const e=document.getElementById("toggle-theme-btn");if(e){const o=e.cloneNode(!0);e.parentNode.replaceChild(o,e)}const{currentUser:a,currentBudget:r,budgets:d=[]}=window.appState||{},[t,n,l,i]=await Promise.all([a?u(a.uid):[],r?y(r.id):[],r?f([...new Set([r.userId,...r.usuariosPermitidos||[]])]):[],r?h(r.userId):null]),m=i?.email||i?.displayName||"Proprietário",p={currentUser:a,currentBudget:r,budgets:d,usersWithAccess:l.filter(o=>o.uid!==r?.userId),ownerDisplay:m,pendingInvitations:t,sentInvitations:n,appVersion:"v4.38.0",hasNewChangelog:!0,lastUpdateLabel:"16/09/2025",txChunkOverride:localStorage.getItem("txChunkSize")||"",perfEnabled:!1};s.innerHTML=E(p);try{console.log("[DEBUG] Verificando attachDynamicHandlers:",typeof window.attachDynamicHandlers),typeof window.attachDynamicHandlers=="function"?(console.log("[DEBUG] Chamando attachDynamicHandlers para content:",s),window.attachDynamicHandlers(s),console.log("[DEBUG] attachDynamicHandlers executado com sucesso")):console.warn("[SettingsPage] attachDynamicHandlers ausente; cliques delegados podem não funcionar")}catch(o){console.warn("[SettingsPage] Erro ao executar attachDynamicHandlers:",o)}try{console.log("[DEBUG] Verificando setupGlobalHandlers:",typeof window.setupGlobalHandlers),typeof window.setupGlobalHandlers=="function"?(console.log("[DEBUG] Chamando setupGlobalHandlers..."),window.setupGlobalHandlers(),console.log("[DEBUG] setupGlobalHandlers executado com sucesso")):console.warn("[SettingsPage] setupGlobalHandlers ausente; prosseguindo sem inicialização adicional")}catch(o){console.warn("[SettingsPage] Erro ao executar setupGlobalHandlers:",o)}j()}catch(e){console.error("☠️ Falha catastrófica ao renderizar SettingsPage:",e),s.innerHTML=`<div class="empty-state"><div class="empty-icon">☠️</div><div class="empty-text">Erro ao carregar configurações</div><div class="empty-description">${e.message}</div></div>`}}function j(){try{window.initializeTheme&&window.initializeTheme(),[{id:"help-support-btn",fn:window.openHelp},{id:"rate-app-btn",fn:window.rateApp},{id:"copy-info-btn",fn:window.copyAppInfo},{id:"clear-cache-btn",fn:window.clearOfflineCache},{id:"whats-new-btn",fn:window.showWhatsNew},{id:"install-app-btn",fn:window.installApp}].forEach(({id:r,fn:d})=>{const t=document.getElementById(r);t&&typeof d=="function"&&(t.onclick=d)});let a=document.getElementById("toggle-theme-btn")||document.getElementById("theme-toggle-btn");if(a){a.id="theme-toggle-btn";const r=a.cloneNode(!0);a.parentNode.replaceChild(r,a),a=r}v(()=>import("./main-kf7gv-aY.js").then(r=>r.aa),__vite__mapDeps([0,1])).then(r=>{typeof r.setupThemeToggle=="function"?(r.setupThemeToggle("theme-toggle-btn"),console.log("[DEBUG] setupThemeToggle conectado ao botão #theme-toggle-btn")):console.warn("[DEBUG] setupThemeToggle não encontrado no módulo global")})}catch(e){console.warn("Falha no setup pós-renderização",e)}[{id:"limit-alerts-toggle",key:"noti_limit_alerts",label:"Alertas de Limite"},{id:"recurring-reminders-toggle",key:"noti_recurring_reminders",label:"Lembretes de Recorrentes"},{id:"weekly-summary-toggle",key:"noti_weekly_summary",label:"Resumo Semanal"}].forEach(({id:e,key:a,label:r})=>{const d=document.getElementById(e);if(d){const t=localStorage.getItem(a);t!==null&&(d.checked=t==="true"),d.addEventListener("change",()=>{localStorage.setItem(a,d.checked),window.Snackbar?window.Snackbar.success(`${r} ${d.checked?"ativado":"desativado"}`):console.log(`[Notificações] ${r}:`,d.checked)})}})}c.on("auth:changed",()=>g());c.on("budget:changed",()=>g());c.on("invitation:changed",()=>g());function g(){window.location.hash.startsWith("#/settings")&&A()}export{A as renderSettings};

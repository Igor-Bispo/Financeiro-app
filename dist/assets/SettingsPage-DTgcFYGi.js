const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/main-C6mVWVKS.js","assets/main-CF1rWTKC.css"])))=>i.map(i=>d[i]);
import{_ as v,e as l}from"./main-C6mVWVKS.js";import{loadBudgetInvitations as u,loadSentBudgetInvitations as y,fetchUsersInfo as f,fetchUserInfo as h}from"./settings.service-COWRWlEF.js";function m(s){s?(document.body.classList.remove("no-animations"),localStorage.setItem("animationsEnabled","1")):(document.body.classList.add("no-animations"),localStorage.setItem("animationsEnabled","0"))}document.addEventListener("DOMContentLoaded",()=>{const s=localStorage.getItem("animationsEnabled"),e=s===null?!0:s==="1";m(e);const r=document.getElementById("animations-toggle");r&&(r.checked=e,r.addEventListener("change",a=>{m(a.target.checked)}))});function b(s,e,r){return`
    <div class="empty-state">
        <div class="empty-icon">${s}</div>
        <div class="empty-text">${e}</div>
        <div class="empty-description">${r}</div>
    </div>
    `}function k(s,e,r){const a=e?.uid;console.log("[DEBUG] renderSharedWithMe - budgets:",s),console.log("[DEBUG] renderSharedWithMe - currentUser:",e),console.log("[DEBUG] renderSharedWithMe - me (uid):",a);const d=(s||[]).filter(t=>{const i=t&&(t.isOwner===!1||a&&t.userId&&t.userId!==a);return console.log("[DEBUG] Orçamento:",t.nome,"isOwner:",t.isOwner,"userId:",t.userId,"me:",a,"isShared:",i),i});return console.log("[DEBUG] Orçamentos compartilhados filtrados:",d),d.length?`
        <div class="space-y-4">
            ${d.map(t=>`
                <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                    <div class="p-5">
                        <div class="flex justify-between items-start gap-4">
                            ...
                            ${t.id!==r?.id?`
                                <button type="button" class="enter-budget-button btn btn-primary btn-sm" data-budget-id="${t.id}" data-budget-name="${(t.nome||"Orçamento").replace(/\"/g,"&quot;")}" title="Entrar neste orçamento">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-800/50 px-5 py-3 flex justify-end items-center gap-2">
                        <button class="btn btn-ghost btn-sm copy-budget-id-btn" data-budget-id="${t.id}" title="Copiar ID do orçamento">📋 Copiar ID</button>
                        <button class="btn btn-danger btn-sm leave-budget-btn" data-budget-id="${t.id}" data-budget-name="${(t.nome||"Orçamento").replace(/"/g,"&quot;")}" title="Sair deste orçamento">🚪 Sair</button>
                    </div>
                </div>
            `).join("")}
        </div>
    `:b("🤝","Nenhum orçamento compartilhado","Convites que você aceitar aparecerão aqui.")}function w(s,e,r){const a=e?.uid,d=(s||[]).filter(t=>t&&t.isOwner!==!1&&(!a||t.userId===a));return d.length?`
        <div class="space-y-4">
            ${d.map(t=>`
                <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border ${t.id===r?.id?"border-blue-500":"border-gray-200 dark:border-gray-700"} overflow-hidden">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-800/50 px-5 py-3 flex justify-end items-center gap-2">
                        <button class="btn btn-ghost btn-sm copy-budget-id-btn" data-budget-id="${t.id}" title="Copiar ID do orçamento">📋 Copiar ID</button>
                        <button class="btn btn-danger btn-sm delete-budget-btn" data-budget-id="${t.id}" data-budget-name="${(t.nome||"Orçamento").replace(/"/g,"&quot;")}" title="Excluir orçamento">🗑️ Excluir</button>
                    </div>
                </div>
            `).join("")}
        </div>
    `:b("🗂️","Nenhum orçamento próprio","Crie um novo orçamento para começar.")}function S(s){return!s||s.length===0?`
            ...
            <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
                <div class="flex items-center justify-between mb-2 md:mb-0 gap-4">
                    ...
                    <div class="flex-shrink-0 ml-2">
                        <button class="remove-user-btn btn btn-danger btn-sm" data-uid="${e.uid}" data-email="${e.email}" title="Remover usuário">
    ...
    `}function E(s){return!s||s.length===0?`
        <div class="text-center py-8">
            <div class="text-4xl mb-3">📤</div>
    ...
    <div class="space-y-3">
            ...
            <div class="bg-orange-50 dark:bg-orange-900/20 rounded-lg border border-orange-200 dark:border-orange-800 p-3" data-invite-id="${e.id}">
                <div class="flex items-center justify-between mb-2 md:mb-0 gap-4">
                    <div class="flex items-center gap-3 min-w-0 flex-1">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="font-medium text-gray-900 dark:text-gray-100 truncate text-sm">${e.invitedUserEmail||e.email||"Email não disponível"}</div>
                    ...
                    <div class="flex-shrink-0 ml-2">
                        <button class="cancel-invitation-btn btn btn-outline btn-sm" data-invite-id="${e.id}" title="Cancelar convite">
        ...
        `).join("")}
    </div>`}function C(s){return!s||s.length===0?`
        <div class="text-center py-8">
            <div class="text-4xl mb-3">📬</div>
            <div class="text-gray-500 dark:text-gray-400">Nenhum convite pendente</div>
        ...
        ${s.map(e=>`
            <div class="bg-purple-50 dark:bg-purple-900/20 rounded-lg border border-purple-200 dark:border-purple-800 p-3" data-invite-id="${e.id}" data-budget-id="${e.budgetId}">
                ...
                <div class="flex gap-2">
                    <button class="accept-invitation-btn btn btn-primary btn-sm flex-1" data-invite-id="${e.id}" title="Aceitar convite">
                        ✅ Aceitar
                    </button>
                    <button class="decline-invitation-btn btn btn-danger btn-sm flex-1" data-invite-id="${e.id}" title="Recusar convite">
                        ❌ Recusar
                    </button>
                </div>
            </div>
        `).join("")}
    </div>`}function A(s){const{currentUser:e,currentBudget:r,budgets:a,usersWithAccess:d,ownerDisplay:t,pendingInvitations:i,sentInvitations:o}=s;return`
              ...
              <div class="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div class="flex items-center gap-1">
                    <span class="text-gray-600 dark:text-gray-400 text-xs">${e?.displayName||e?.email?.split("@")[0]||"Usuário"}</span>
                  </div>
                </div>
              </div>
            </div>
                        ...
                        <div class="bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-gray-800 dark:via-gray-700 dark:to-gray-800 rounded-xl shadow-lg border border-blue-200 dark:border-gray-600 p-2">
                            ...
                            <div class="flex items-center justify-between mb-2 gap-4">
                                ...
                                <div class="flex items-center gap-1 text-xs text-gray-600 dark:text-gray-400">
                                    <span>${r?.nome||"Orçamento Principal"}</span>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-1">
                                ...
                                <div class="bg-white dark:bg-gray-800 rounded-md p-2 shadow-sm border border-gray-200 dark:border-gray-600">
                                    <div class="flex items-center justify-between gap-4">
                                        <div class="flex items-center gap-1">
                                            ...
                                            <span class="text-xs font-medium text-gray-800 dark:text-gray-200">${e?.displayName||e?.email?.split("@")[0]||"Usuário"}</span>
                                        </div>
                                ...
                                <div class="bg-white dark:bg-gray-800 rounded-md p-2 shadow-sm border border-gray-200 dark:border-gray-600">
                                    <div class="flex flex-col gap-1">
                                        ...
                                        <div class="text-xs text-purple-600 dark:text-purple-400 truncate" title="${t}">
                                            ${t.split("@")[0]||t}
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="flex items-center justify-between mb-4 gap-4">
                                ...
                                <div class="text-right">
                                    <div class="text-lg font-bold text-blue-600 dark:text-blue-400">
                                        ${d.length+(o?.length||0)+(i?.length||0)}
                                    </div>
    ...
    `}async function $(){const s=document.getElementById("app-content");if(!s){console.warn("⚠️ SettingsPage: elemento #app-content não encontrado");return}try{const e=document.getElementById("toggle-theme-btn");if(e){const n=e.cloneNode(!0);e.parentNode.replaceChild(n,e)}const{currentUser:r,currentBudget:a,budgets:d=[]}=window.appState||{},[t,i,o,g]=await Promise.all([r?u(r.uid):[],a?y(a.id):[],a?f([...new Set([a.userId,...a.usuariosPermitidos||[]])]):[],a?h(a.userId):null]),p=g?.email||g?.displayName||"Proprietário",x={currentUser:r,currentBudget:a,budgets:d,usersWithAccess:o.filter(n=>n.uid!==a?.userId),ownerDisplay:p,pendingInvitations:t,sentInvitations:i,appVersion:"v4.42.0",hasNewChangelog:!0,lastUpdateLabel:"16/09/2025",txChunkOverride:localStorage.getItem("txChunkSize")||"",perfEnabled:!1};s.innerHTML=A(x);try{console.log("[DEBUG] Verificando attachDynamicHandlers:",typeof window.attachDynamicHandlers),typeof window.attachDynamicHandlers=="function"?(console.log("[DEBUG] Chamando attachDynamicHandlers para content:",s),window.attachDynamicHandlers(s),console.log("[DEBUG] attachDynamicHandlers executado com sucesso")):console.warn("[SettingsPage] attachDynamicHandlers ausente; cliques delegados podem não funcionar")}catch(n){console.warn("[SettingsPage] Erro ao executar attachDynamicHandlers:",n)}try{console.log("[DEBUG] Verificando setupGlobalHandlers:",typeof window.setupGlobalHandlers),typeof window.setupGlobalHandlers=="function"?(console.log("[DEBUG] Chamando setupGlobalHandlers..."),window.setupGlobalHandlers(),console.log("[DEBUG] setupGlobalHandlers executado com sucesso")):console.warn("[SettingsPage] setupGlobalHandlers ausente; prosseguindo sem inicialização adicional")}catch(n){console.warn("[SettingsPage] Erro ao executar setupGlobalHandlers:",n)}I()}catch(e){console.error("☠️ Falha catastrófica ao renderizar SettingsPage:",e),s.innerHTML=`<div class="empty-state"><div class="empty-icon">☠️</div><div class="empty-text">Erro ao carregar configurações</div><div class="empty-description">${e.message}</div></div>`}}function I(){try{window.initializeTheme&&window.initializeTheme(),[{id:"help-support-btn",fn:window.openHelp},{id:"rate-app-btn",fn:window.rateApp},{id:"copy-info-btn",fn:window.copyAppInfo},{id:"clear-cache-btn",fn:window.clearOfflineCache},{id:"whats-new-btn",fn:window.showWhatsNew},{id:"install-app-btn",fn:window.installApp}].forEach(({id:a,fn:d})=>{const t=document.getElementById(a);t&&typeof d=="function"&&(t.onclick=d)});let r=document.getElementById("toggle-theme-btn")||document.getElementById("theme-toggle-btn");if(r){r.id="theme-toggle-btn";const a=r.cloneNode(!0);r.parentNode.replaceChild(a,r),r=a}v(()=>import("./main-C6mVWVKS.js").then(a=>a.ab),__vite__mapDeps([0,1])).then(a=>{typeof a.setupThemeToggle=="function"?(a.setupThemeToggle("theme-toggle-btn"),console.log("[DEBUG] setupThemeToggle conectado ao botão #theme-toggle-btn")):console.warn("[DEBUG] setupThemeToggle não encontrado no módulo global")})}catch(e){console.warn("Falha no setup pós-renderização",e)}[{id:"limit-alerts-toggle",key:"noti_limit_alerts",label:"Alertas de Limite"},{id:"recurring-reminders-toggle",key:"noti_recurring_reminders",label:"Lembretes de Recorrentes"},{id:"weekly-summary-toggle",key:"noti_weekly_summary",label:"Resumo Semanal"}].forEach(({id:e,key:r,label:a})=>{const d=document.getElementById(e);if(d){const t=localStorage.getItem(r);t!==null&&(d.checked=t==="true"),d.addEventListener("change",()=>{localStorage.setItem(r,d.checked),window.Snackbar?window.Snackbar.success(`${a} ${d.checked?"ativado":"desativado"}`):console.log(`[Notificações] ${a}:`,d.checked)})}})}l.on("auth:changed",()=>c());l.on("budget:changed",()=>c());l.on("invitation:changed",()=>c());function c(){window.location.hash.startsWith("#/settings")&&$()}export{$ as renderSettings};

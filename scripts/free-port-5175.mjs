...
async function killOnWindows() {
  // Find PIDs listening on port using netstat and kill them
  try {
    // Using cmd's findstr for compatibility
    const { stdout } = await exec(`netstat -ano -p tcp | findstr :${PORT}`);
    const lines = stdout.split(/\r?\n/).map(l => l.trim()).filter(Boolean);
    const pids = new Set();
    for (const line of lines) {
      // Example:
      // TCP    0.0.0.0:5175   0.0.0.0:0   LISTENING   12345
      // TCP    [::]:5175      [::]:0      LISTENING   12345
    ...
    for (const pid of pids) {
      // Try to fetch process details for better guidance
      let procInfo = null;
      try {

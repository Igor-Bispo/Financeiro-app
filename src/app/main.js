  ...
  try {
    ...
    try {
      const ensureDashboard = () => {
        ...
        const h = raw.split('?')[0];
        // ForÃ§ar dashboard em: vazio, raiz, desconhecida ou recorrentes (pedido do usuÃ¡rio)
        if (!h || h === '#' || h === '#/' || !allowed.has(h) || h === '#/recorrentes') {
          window.location.hash = '#/dashboard';
        }
        // Guardar uma janela curta em que recusamos navegaÃ§Ã£o automÃ¡tica para recorrentes
    ...
    async function onRoute() {
      ...
      // ForÃ§ar scroll para o topo na troca de rota (antes de render)
      try { const { scrollToTop } = await import('@core/utils/globalUtils.js'); scrollToTop(true); } catch {}
      // Sincronizar perÃ­odo com ?ym no hash
      try {
      ...
      // Se ainda dentro da janela de proteÃ§Ã£o e tentando ir para recorrentes, volte para dashboard
      ...
      try {
        ...
        // Scroll serÃ¡ tratado dentro do pipeline de render (routes.js)
      } catch (err) {
        logger.error('route error', err);
      }
    }

    // Reagir a mudanÃ§as de hash normalmente
    ...
    // Evitar restauraÃ§Ã£o automÃ¡tica de rolagem pelo navegador
    try {
    ...
    logger.info('AplicaÃ§Ã£o bootstrapada com sucesso');

...
export const routes = {
  ...
  // Análises: usar a página real via shim de features (que delega p/ js/ui/AnalyticsPage)
  '/analytics': () => import('@features/analytics/AnalyticsPage.js'),
  '/notifications': () => import('@features/notifications/NotificationsPage.js'),
  // Configurações: usar a página real de settings
  '/settings': () => import('@features/settings/SettingsPage.js'),
};

// Função para renderizar página baseada na rota
export async function renderPage(route) {
  try {
    ...
    console.log('🔄 Renderizando página:', effectiveRoute, 'Rota original:', route);
    ...
    if (routes[effectiveRoute]) {
      ...
      if (typeof renderFunction === 'function') {
        ...
        // Garantir FAB visível após render da página
        try {
        ...
        try {
          ...
          if (!window.swipeNavigation) {
            console.log('🔄 Criando nova instância do SwipeNavigation...');
            window.swipeNavigation = new SwipeNavigation();
            console.log('✅ SwipeNavigation criado:', window.swipeNavigation);
          } else {
            console.log('🔄 Atualizando SwipeNavigation existente...');
            try { window.swipeNavigation.updateCurrentTabIndex(); } catch {}
            try { window.swipeNavigation.updateSwipeIndicator(); } catch {}
          }
        } catch (e) {
          console.warn('❌ Swipe Navigation não pôde ser inicializado/atualizado:', e);
        }
        console.log('✅ Página renderizada:', effectiveRoute);
      } else {
        console.error('❌ Função de renderização não encontrada para:', effectiveRoute);
      }
    } else {
      console.warn('⚠️ Rota não encontrada:', effectiveRoute);
      ...
      if (routes['/dashboard']) {
        ...
        // Garantir Swipe Navigation também no fallback
        try {
          const { SwipeNavigation } = await import('@js/ui/SwipeTabs.js');
          if (!window.swipeNavigation) {
            window.swipeNavigation = new SwipeNavigation();
          ...
          console.warn('Swipe Navigation (fallback) não pôde ser inicializado:', e);
        }
      }
    }
  } catch (error) {
    console.error('❌ Erro ao renderizar página:', route, error);
    ...
    try {
      ...
      if (routes['/dashboard']) {
        ...
        try {
          ...
          console.warn('Swipe Navigation (fallback catch) não pôde ser inicializado:', e);
        }
      }
    } catch (fallbackError) {
      console.error('❌ Erro no fallback para dashboard:', fallbackError);
    }
  }
}

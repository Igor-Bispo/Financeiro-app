...
export async function corrigirCategoriasSemBudget(options = {}) {
  try {
    ...
    if (!user || !budget) {
      if (!options.silent && window.Snackbar) window.Snackbar({ message: 'Faça login e selecione um orçamento.', type: 'error' });
      return { corrigidas: 0 };
    }
    ...
    for (const d of snap.docs) {
      ...
      if (!data.budgetId) {
        if (multiBudget) {
          // Em múltiplos orçamentos, não arriscar atribuição errada automaticamente
          continue;
        } else {
    ...
    if (!options.silent && window.Snackbar) {
      const msg = (Array.isArray(window.appState?.budgets) && window.appState.budgets.length > 1 && corrigidas === 0)
        ? 'Categorias antigas sem budgetId foram mantidas (múltiplos orçamentos).'
...
export async function corrigirTransacoesSemBudget(options = {}) {
  try {
    ...
    const multiBudget = Array.isArray(window.appState?.budgets) && window.appState.budgets.length > 1;
    // Construir mapa categoriaId -> budgetId para inferência segura
    let catMap = new Map();
    try {
    ...
    for (const d of snap.docs) {
      ...
      if (!data.budgetId) {
        if (multiBudget) {
          // Em múltiplos orçamentos, atribuir apenas se houver categoria com budgetId conhecido
          ...
          if (inferred) {
            ...
            // Sem inferência, não atribuir para evitar vazamento entre orçamentos
          }
        } else {
          await updateDoc(d.ref, { budgetId: budget.id, updatedAt: serverTimestamp() });
          corrigidas++;
    ...
    if (!options.silent && window.Snackbar) {
      const msg = (multiBudget && corrigidas === 0)
        ? 'Transações antigas sem budgetId foram mantidas (múltiplos orçamentos).'
        : `${corrigidas} transações corrigidas com budgetId`;
      window.Snackbar({ message: msg, type: 'success' });
    }
    return { corrigidas };
  } catch (e) {
    console.error('Erro ao corrigir transações sem budgetId:', e);
    if (!options.silent && window.Snackbar) window.Snackbar({ message: 'Erro ao corrigir transações', type: 'error' });
    return { corrigidas: 0, error: e };
  }
}
...
export async function corrigirRecorrentesSemBudget(options = {}) {
  try {
    ...
    const multiBudget = Array.isArray(window.appState?.budgets) && window.appState.budgets.length > 1;
    // Mapa de categorias para inferência
    let catMap = new Map();
    try {
      const allCats = await categoriesRepo.list({ userId: user.uid });
      for (const c of allCats || []) {
      ...
      if (!data.budgetId) {
        if (multiBudget) {
          ...
          if (inferred) {
            ...
            // sem inferência segura, não atribuir
          }
        } else {
          await updateDoc(d.ref, { budgetId: budget.id, updatedAt: serverTimestamp() });
          corrigidas++;
    ...
    if (!options.silent && window.Snackbar) {
      const msg = (multiBudget && corrigidas === 0)
        ? 'Recorrentes antigas sem budgetId foram mantidas (múltiplos orçamentos).'
...
export async function corrigirTudoSemBudget(options = {}) {
  try {
    if (!options.silent && window.Snackbar) window.Snackbar({ message: 'Iniciando correção de dados antigos...', type: 'info' });
    ...
    if (!options.silent && window.Snackbar) window.Snackbar({ message: 'Correção concluída.', type: 'success' });
    return { categorias: r1.corrigidas, transacoes: r2.corrigidas, recorrentes: r3.corrigidas };
  } catch (e) {
    console.error('Erro ao executar correções:', e);
    if (!options.silent && window.Snackbar) window.Snackbar({ message: 'Erro ao executar correções', type: 'error' });
...
export async function migrarOrfaosPorCategoria(options = {}) {
  try {
    ...
    if (!options.silent && window.Snackbar) window.Snackbar({ message: `Migração: ${txCorr} transações e ${recCorr} recorrentes atribuídas por categoria`, type: 'success' });
    return { tx: txCorr, rec: recCorr };
  } catch (e) {
    console.warn('Migração global por categoria falhou:', e);
    if (!options.silent && window.Snackbar) window.Snackbar({ message: 'Erro na migração global', type: 'error' });
...
export async function corrigirOrfaosDoOrcamento(options = {}) {
  try {
    ...
    if (!options.silent && window.Snackbar) window.Snackbar({ message: `Correção do orçamento: ${txCorr} transações e ${recCorr} recorrentes atribuídas`, type: 'success' });
    return { tx: txCorr, rec: recCorr };
  } catch (e) {
    console.warn('Correção por orçamento falhou:', e);
    if (!options.silent && window.Snackbar) window.Snackbar({ message: 'Erro na correção do orçamento', type: 'error' });
    return { tx: 0, rec: 0, error: e };
  }
}

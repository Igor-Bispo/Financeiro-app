// UtilitÃ¡rios globais para substituir funÃ§Ãµes do window
...
export function updatePageTitle(path) {
  try {
    ...
    document.title = `Financeiro â€¢ ${title}`;
...
// FunÃ§Ãµes de compatibilidade para substituir window.*
...
// Helpers de sincronizaÃ§Ã£o de perÃ­odo <-> hash
...
// Sistema de funÃ§Ãµes globais (substituindo window.*)
export const globalFunctions = {
  showAddTransactionModal: async (data) => {
    try {
      ...
      eventBus.emit('modal:show', { type: 'transaction', data, title: 'Nova TransaÃ§Ã£o' });
    } catch (e) {
      console.warn('Falha ao abrir modal de transaÃ§Ã£o via shim:', e);
      eventBus.emit('modal:show', { type: 'transaction', data, title: 'Nova TransaÃ§Ã£o' });
    }
  },
...
if (typeof window !== 'undefined') {
  // Substituir funÃ§Ãµes do window por versÃµes modernas

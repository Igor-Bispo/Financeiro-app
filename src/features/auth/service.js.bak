...
export function waitForAuth() {
  return new Promise((resolve) => {
    const start = () => onAuthStateChanged(auth, (user) => {
      ...
      // Garantir que o perfil do usuário esteja salvo/atualizado
      if (user) {
      ...
      // Parar de escutar após primeira autenticação
      unsubscribe();
    });
    let unsubscribe;
    authReady
...
// Garante que o documento do usuário exista/esteja atualizado em /users/{uid}
export async function ensureUserProfile(user) {
  try {
    ...
    console.warn('ensureUserProfile: falha ao salvar perfil do usuário:', err?.message || err);
  }
}

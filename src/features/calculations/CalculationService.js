...
// FunÃ§Ã£o para calcular saldo do orÃ§amento
export function calculateBudgetBalance(budgetId) {
  try {
    ...
    console.error('âŒ Erro ao calcular saldo do orÃ§amento:', error);
    return { receitas: 0, despesas: 0, saldo: 0, orcado: 0 };
  }
}

// FunÃ§Ã£o para calcular totais por categoria
export function calculateCategoryTotals(budgetId, month = null) {
  try {
    ...
    console.error('âŒ Erro ao calcular totais por categoria:', error);
    return [];
  }
}

// FunÃ§Ã£o para calcular estatÃ­sticas mensais
export function calculateMonthlyStats(budgetId, months = 6) {
  try {
    ...
    console.error('âŒ Erro ao calcular estatÃ­sticas mensais:', error);
    return [];
  }
}

// FunÃ§Ã£o para calcular tendÃªncias
export function calculateTrends(budgetId, months = 3) {
  try {
    ...
    console.error('âŒ Erro ao calcular tendÃªncias:', error);
    return { trend: 'estavel', percentage: 0 };
  }
}

// FunÃ§Ã£o para calcular projeÃ§Ãµes
export function calculateProjections(budgetId, months = 3) {
  try {
    ...
    // Calcular mÃ©dia de crescimento
    const changes = [];
    ...
    for (let i = 1; i <= months; i++) {
      ...
      projections.push({
        ...
        confidence: Math.max(0, 100 - (i * 20)) // ConfianÃ§a diminui com o tempo
      });
    }
    ...
    console.error('âŒ Erro ao calcular projeÃ§Ãµes:', error);
    return null;
  }
}

// FunÃ§Ã£o auxiliar para arredondar para 2 casas decimais

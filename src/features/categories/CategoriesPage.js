...
// (helper getTransactionYearMonth nÃ£o Ã© necessÃ¡rio aqui)

export async function render(container) {
  const root = document.createElement('div');
  root.className = 'categories-page';

  // Header com indicador de perÃ­odo
  const header = document.createElement('div');
  header.className = 'tab-header mb-6';
  header.innerHTML = '<h2 class="tab-title-highlight">ðŸ“‚ Categorias</h2><div id="cat-period-indicator"></div>';

  // O indicador de perÃ­odo serÃ¡ injetado na renderizaÃ§Ã£o principal (cat-period-indicator)

  root.appendChild(header);

  // ConteÃºdo das categorias
  ...
  content.innerHTML = `
    <div class="u-card bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 p-6">
      <div class="flex justify-between items-center mb-6">
        <h3 class="text-lg font-bold text-gray-900 dark:text-white">ðŸ“ Gerenciar Categorias</h3>
  ...
  // Empty state quando nÃ£o hÃ¡ categorias
  try {
    ...
    if (wrap && !hasAny) {
      ...
      empty.innerHTML = '<div class="empty-icon">ðŸ“</div><div class="empty-text">Nenhuma categoria cadastrada</div><div class="mt-2"><button class="u-btn u-btn--primary mobile-btn w-full" onclick=\"window.showAddCategoryModal && window.showAddCategoryModal()\">Criar categoria</button></div>';
      wrap.appendChild(empty);
    }
  } catch {}

  // Garantir indicador apÃ³s render
...
// FunÃ§Ã£o para renderizar categorias (movida do app.js)
export async function renderCategories() {
  ...
  try {
    ...
    if (window.__lastCategoriesRender && (now - window.__lastCategoriesRender) < 300) {
      console.log('â±ï¸ renderCategories chamado muito prÃ³ximo do anterior, pulando...');
      return;
    }
    window.__lastCategoriesRender = now;
  } catch {}
  // Garantir dados necessÃ¡rios disponÃ­veis
  await loadTransactions();
  await loadRecorrentes();
  const content = document.getElementById('app-content');

  // Calcular estatÃ­sticas das categorias
  ...
  // Calcular gastos por categoria no mÃªs selecionado
  const { year: anoAtual, month: mesAtual } = getSelectedPeriod();

  const categoriasComGastos = window.appState.categories
    .map(cat => {
      // Filtrar transaÃ§Ãµes da categoria no mÃªs atual (incluindo recorrentes aplicadas)
      ...
      // Total geral (transaÃ§Ãµes + recorrentes)
  ...
  // Totais agregados (Despesas): Limite total, Gasto do mÃªs e Saldo
  ...
  // Totais agregados (Receitas): Meta total, Recebido do mÃªs e Saldo
  ...
  content.innerHTML = `
    <div class="tab-container">
      <div class="tab-header">
        <h2 class="tab-title-highlight">ðŸ“‚ Categorias</h2>
        ...
        <div class="content-spacing">
            ...
            <div class="bg-gradient-to-br from-purple-500 via-pink-500 to-indigo-600 rounded-2xl shadow-xl p-6 md:p-8 text-white mb-6">
              <!-- Header do Card -->
                ...
                <div class="text-right">
                  ...
                  <p class="text-xs opacity-90">${categoriasEmAlerta > 0 ? 'âš ï¸ Alertas' : 'âœ… Sem Alertas'}</p>
                </div>
              </div>
          ...
          <!-- Filtro de pesquisa -->
            ...
            <div class="relative">
              <input 
                type="text" 
                id="category-search" 
                    placeholder="ðŸ” Pesquisar por nome, tipo ou limite..." 
          ...
          <div class="mb-8" id="categories-grid">
            <div class="flex items-center gap-2 mb-4">
              ...
              <h2 class="text-xl md:text-2xl font-bold text-gray-900 dark:text-gray-100">ðŸ“‹ Todas as Categorias</h2>
              </div>
...
${categorias.length === 0 ? `
                    ...
                    <div class="u-card bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-2xl transition-all duration-300 group ${excedeuLimite ? 'ring-2 ring-red-200 dark:ring-red-800' : ''}">
                      ...
                      <div class="bg-gradient-to-r ${isReceita ? 'from-green-50 to-emerald-50 dark:from-gray-800 dark:to-gray-800' : 'from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-800'} p-4 border-b border-gray-200 dark:border-gray-700">
                        <div class="flex items-center justify-between">
                          ...
                          ${excedeuLimite ? '<div class="text-2xl">ðŸš¨</div>' : temLimite && cat.porcentagem >= 90 ? '<div class="text-2xl">âš ï¸</div>' : ''}
                        </div>
                  </div>
                        ...
                        ${temLimite ? `
                          ...
                          <div class="space-y-2 mb-4">
                            <div class="flex justify-between items-center">
                              <span class="text-gray-600 dark:text-gray-400">${cat.tipo === 'receita' ? 'Receita' : 'Gasto'} do mÃªs:</span>
  ...
  cat.totalGasto > 0
    ? `
                                  <div class="text-xs text-gray-500 dark:text-gray-400 pl-2">
                                    â€¢ ${cat.tipo === 'receita' ? 'Receitas' : 'TransaÃ§Ãµes'}: R$ ${cat.totalGastoTransacoes.toFixed(2)}
                                    ${cat.totalGastoRecorrentes > 0 ? `<br>â€¢ Recorrentes: R$ ${cat.totalGastoRecorrentes.toFixed(2)}` : ''}
                                  </div>
                                `
    : ''
}
                            <div class="text-xs text-gray-500 dark:text-gray-400">
                              ${excedeuLimite ? `ðŸš¨ Excedeu em R$ ${Math.abs(cat.saldo).toFixed(2)}` : `Saldo: R$ ${cat.saldo.toFixed(2)}`}
                            </div>
                          </div>
                        ` : `
                          <div class="space-y-2 mb-4">
  ...
  // Configurar botÃµes da tela de categorias
  setTimeout(() => {
    try { setupCategoryButtons(); } catch (e) { console.warn('setupCategoryButtons falhou:', e); }
    // Bind seguro para o botÃ£o de adicionar
    try {
  ...
  // Fallbacks globais para aÃ§Ãµes da pÃ¡gina de categorias (evitar undefined)
  try {
    if (typeof window.deleteCategoryWithConfirmation !== 'function') {
      window.deleteCategoryWithConfirmation = async function(categoryId, categoryName = 'categoria') {
        try {
          ...
          const ok = confirm(`Excluir "${name}"? Esta aÃ§Ã£o nÃ£o pode ser desfeita.`);
          if (!ok) return;
          const svc = await import('./service.js');
          await svc.deleteCategory(categoryId);
          try { window.Snackbar?.show?.('Categoria excluÃ­da', 'success'); } catch {}
    ...
    if (typeof window.showCategoryHistory !== 'function') {
      window.showCategoryHistory = function(categoryId) {
        try {
          // HistÃ³rico filtrado pelo perÃ­odo atual, com parsing robusto de datas
          const { year, month } = getSelectedPeriod();
          const cat = window.appState?.categories?.find(c => c.id === categoryId);
          const title = `HistÃ³rico - ${cat?.nome || categoryId}`;
          ...
          const items = txs
            ...
            .map(t => {
              ...
              const desc = (t.descricao && String(t.descricao).trim()) || '(Sem descriÃ§Ã£o)';
            ...
            .join('') || '<div class=\"text-sm text-gray-500\">Sem transaÃ§Ãµes neste perÃ­odo</div>';
...
// Loader local de transaÃ§Ãµes: usa serviÃ§o legado se existir, senÃ£o mantÃ©m estado atual
async function loadTransactions() {
  try {
    ...
    // Fallback: nada a fazer, assumimos que appState jÃ¡ tem transaÃ§Ãµes
  } catch (e) {
    console.warn('loadTransactions fallback falhou:', e);
  }
}

// Loader local de recorrentes usando o serviÃ§o da feature
...
function setupCategorySearch() {
  try {
    ...
    // Helpers locais de normalizaÃ§Ã£o e fuzzy
...
// FunÃ§Ã£o para renderizar todas as categorias
export function renderAllCategories() {
  ...
  cat.totalGasto > 0
    ? `
                    <div class="text-xs text-gray-500 dark:text-gray-400 pl-2">
                      â€¢ TransaÃ§Ãµes: R$ ${cat.totalGastoTransacoes.toFixed(2)}
                      ${cat.totalGastoRecorrentes > 0 ? `<br>â€¢ Recorrentes: R$ ${cat.totalGastoRecorrentes.toFixed(2)}` : ''}
                    </div>
                  `
    : ''
...
// FunÃ§Ã£o para renderizar categorias filtradas

...
export function renderDashboard(container) {
  // Render somente se a rota ativa for Dashboard (quando chamado sem container explÃ­cito)
  try {
    if (!container) {
  ...
  // target Ã© resolvido a cada render
  ...
  if (!listenersAttached) {
    try {
      ...
      // Atualizar tambÃ©m quando abas emitirem atualizaÃ§Ã£o explÃ­cita de resumo
  ...
  // OrÃ§amento: usar valorTotal do orÃ§amento atual, ou somar limites de categorias de despesa
  var orcado = 0;
  try {
  ...
  // Gasto por categoria no mÃªs para calcular alertas (>=70% do limite)
  ...
  html += '    <h2 class="tab-title-highlight">ðŸ“Š Dashboard</h2>';
  ...
  html += '          <h2 class="text-xl md:text-2xl font-bold text-gray-900 dark:text-gray-100">ðŸ“ˆ Resumo Financeiro</h2>';
  html += '        </div>';
  ...
  html += '            <div><h3 class="text-xl md:text-2xl font-bold">VisÃ£o Geral</h3>';
  html += '            <p class="text-sm opacity-90">' + monthName + ' ' + year + ' â€¢ ' + String(totalTransacoes) + ' transaÃ§Ãµes</p></div>';
  html += '            <div class="text-right">';
  html += '              <div class="text-2xl md:text-3xl font-bold ' + (saldo >= 0 ? 'text-green-200' : 'text-red-200') + '">R$ ' + formatBR(saldo) + '</div>';
  html += '              <p class="text-xs opacity-90">' + (saldo >= 0 ? 'âœ“ Saldo Positivo' : 'âš ï¸ Saldo Negativo') + '</p>';
  html += '            </div>';
  html += '          </div>';
  html += '          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">';
  html += '            <div class="rounded-xl p-5 bg-white/15 backdrop-blur-sm flex items-center justify-between">';
  html += '              <div class="flex items-center gap-3"><span class="text-2xl">ðŸ’š</span><span class="text-sm opacity-90">Receitas</span></div>';
  html += '              <div class="text-2xl md:text-3xl font-bold text-green-200">R$ ' + formatBR(receitas) + '</div>';
  html += '            </div>';
  html += '            <div class="rounded-xl p-5 bg-white/15 backdrop-blur-sm flex items-center justify-between">';
  html += '              <div class="flex items-center gap-3"><span class="text-2xl">ðŸ’¸</span><span class="text-sm opacity-90">Despesas</span></div>';
  ...
  html += '              <div class="text-sm font-medium">Progresso do OrÃ§amento</div>';
  ...
  html += '            </div>';
  // Detalhes: valores de Despesas e OrÃ§ado
  html += '            <div class="flex items-center justify-between text-xs opacity-90 mt-2">';
  html += '              <span>Despesas: R$ ' + formatBR(despesas) + '</span>';
  html += '              <span>OrÃ§ado: R$ ' + formatBR(orcado) + '</span>';
  html += '            </div>';
  html += '            <div class="flex items-center justify-between text-xs opacity-90 mt-2">';
  html += '              <span class="' + (totalAlertas>0 ? 'cursor-pointer hover:underline bg-red-500/20 px-2 py-0.5 rounded-full' : '') + '" ' + (totalAlertas>0 ? 'onclick="window.showCategoriasAlertaModal && window.showCategoriasAlertaModal()"' : '') + '>' + (totalAlertas>0 ? ('âš ï¸ ' + totalAlertas + ' alerta(s)') : 'âœ… Tudo OK') + '</span>';
  html += '              <span>' + (saldo >= 0 ? 'ðŸ“ˆ Meta alcanÃ§ada' : 'ðŸ“‰ Revisar gastos') + '</span>';
  html += '            </div>';
  html += '          </div>';
  html += '        </div>';
  html += '      </div>';

  // Top recorrentes (somente despesas recorrentes)
  try {
    ...
    for (var rr=0; rr<tx.length; rr++) {
      ...
      recMap[key].count += 1;
      // Capturar info de parcela quando disponÃ­vel na transaÃ§Ã£o
      try {
        ...
        if (trr.parcelasTotal !== null && trr.parcelasTotal !== undefined) {
          ...
          if (!isNaN(pt)) recMap[key].parcelasTotal = pt;
        }
      } catch {}
    }
    var topRec = [];
    for (var rkey in recMap) { if (Object.prototype.hasOwnProperty.call(recMap, rkey)) topRec.push({ key: rkey, nome: recMap[rkey].nome, total: recMap[rkey].total, count: recMap[rkey].count, parcelaAtual: recMap[rkey].parcelaAtual, parcelasTotal: recMap[rkey].parcelasTotal }); }
    ...
    html += '          <h2 class="text-xl md:text-2xl font-bold text-gray-900 dark:text-gray-100">â™»ï¸ Top Recorrentes</h2>';
    html += '        </div>';
    html += '        <div class="u-card p-4">';
    if (!topRec.length) {
      html += '          <div class="empty-state">\n            <div class="empty-icon">â™»ï¸</div>\n            <div class="empty-text">Sem recorrentes neste perÃ­odo</div>\n            <div class="empty-description">Despesas recorrentes cadastradas aparecerÃ£o aqui</div>\n          </div>';
    } else {
      for (var tr=0; tr<topRec.length; tr++) {
        ...
        try {
          if ((topRec[tr].parcelaAtual !== null && topRec[tr].parcelaAtual !== undefined) && (topRec[tr].parcelasTotal !== null && topRec[tr].parcelasTotal !== undefined)) {
            parcStr = ' <span class="text-xs text-gray-500">â€¢ ' + String(topRec[tr].parcelaAtual) + '/' + String(topRec[tr].parcelasTotal) + '</span>';
          }
        } catch {}
        html += '          <div class="flex items-center justify-between py-2 border-b border-gray-100 dark:border-gray-800 last:border-0">';
        html += '            <div class="font-medium">' + (topRec[tr].nome || 'Recorrente') + ' <span class="text-xs text-gray-500">(' + String(topRec[tr].count) + ')</span>' + parcStr + '</div>';
        html += '            <div class="text-sm text-gray-600 dark:text-gray-300">R$ ' + Number(topRec[tr].total || 0).toFixed(2) + '</div>';
        html += '          </div>';
      }
    }
    html += '        </div>';
  ...
  try {
    ...
    for (var ci2 = 0; ci2 < cats.length; ci2++) {
      ...
      // Gasto do mÃªs nesta categoria, considerando apenas despesas
      var gasto2 = 0;
    ...
    html += '          <h2 class="text-xl md:text-2xl font-bold text-gray-900 dark:text-gray-100">ðŸ“Š Categorias & Limites</h2>';
    html += '        </div>';
    ...
    html += '              <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100">ðŸ† Top 5 Categorias</h3>';
    html += '              <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Categorias com maiores gastos</p>';
    html += '            </div>';
    html += '            <div class="p-4">';
    if (categoriasComGasto.length === 0) {
      html += '              <div class="empty-state">\n                <div class="empty-icon">ðŸ“‚</div>\n                <div class="empty-text">Nenhuma categoria com gastos</div>\n                <div class="empty-description">Suas categorias com despesas aparecerÃ£o aqui</div>\n              </div>';
    } else {
      var medalhas = ['ðŸ¥‡','ðŸ¥ˆ','ðŸ¥‰','4ï¸âƒ£','5ï¸âƒ£'];
    ...
    html += '                <div>';
    html += '                  <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100">âš ï¸ Controle de Limites</h3>';
    ...
    if (!catsLimite.length) {
      ...
      html += '                <div class="text-3xl mb-2">ðŸŽ¯</div>';
      ...
      for (var li=0; li<maxItems; li++) {
        ...
        var icone = 'âœ…';
        if (porcentL >= 100) { corBarraL = 'bg-red-500'; icone = 'ðŸš¨'; }
        else if (porcentL >= 90) { corBarraL = 'bg-red-400'; icone = 'âš ï¸'; }
        else if (porcentL >= 75) { corBarraL = 'bg-yellow-500'; icone = 'âš¡'; }
        else if (porcentL >= 50) { corBarraL = 'bg-orange-500'; icone = 'ðŸ“Š'; }
  ...
  try {
    ...
    html += '          <h2 class="text-xl md:text-2xl font-bold text-gray-900 dark:text-gray-100">ðŸ’³ Atividade Recente</h2>';
    html += '        </div>';
    html += '        <div class="u-card overflow-hidden">';
    html += '          <div class="p-4">';
    html += '            <div class="flex items-center justify-between mb-4">';
    html += '              <div class="text-sm font-medium text-gray-700 dark:text-gray-300">Ãšltimas TransaÃ§Ãµes</div>';
    html += '              <button onclick="window.showAddTransactionModal && window.showAddTransactionModal()" class="u-btn u-btn--primary mobile-btn">+ Nova TransaÃ§Ã£o</button>';
    html += '            </div>';
    if (recent.length === 0) {
      html += '            <div class="empty-state">\n              <div class="empty-icon">ðŸ§¾</div>\n              <div class="empty-text">Sem transaÃ§Ãµes neste perÃ­odo</div>\n              <div class="mt-2"><button class="u-btn u-btn--primary" onclick="window.showAddTransactionModal && window.showAddTransactionModal()">Adicionar transaÃ§Ã£o</button></div>\n            </div>';
    } else {
      for (var r=0;r<recent.length;r++) {
        ...
        html += '                  <div class="font-medium text-gray-900 dark:text-gray-100 truncate">' + (rt.descricao || rt.description || 'TransaÃ§Ã£o') + '</div>';
        html += '                  <div class="text-xs text-gray-500 dark:text-gray-400">' + (cat && cat.nome || 'Sem categoria') + ' â€¢ ' + dStr + parcInfo + '</div>';
        html += '                </div>';
        html += '              </div>';
  ...
  // Mount or update period indicator (reutilizÃ¡vel)
  ...
  try {
    window.showCategoriasAlertaModal = async function() {
      try {
        ...
        // Recalcular gastos por categoria do mÃªs atual

...
export function renderDashboard(container) {
  ...
  html += '    <div class="flex items-center justify-center w-full relative">';
  html += '      <!-- Título centralizado com badge integrado -->';
  html += '      <div class="flex items-center gap-2">';
  html += '        <div class="flex items-center gap-1">';
  html += '          <span class="text-2xl">📊</span>';
  html += '          <h2 class="text-white font-bold text-lg">DASHBOARD</h2>';
  html += '        </div>';
  html += '        <div class="flex items-center gap-1 bg-green-500/20 backdrop-blur-sm px-2 py-1 rounded-full border border-green-400/30">';
  html += '          <span class="w-2 h-2 bg-green-400 rounded-full animate-pulse shadow-sm"></span>';
  html += '          <span class="text-green-600 dark:text-green-300 font-bold text-xs">LIVE</span>';
  html += '        </div>';
  html += '      </div>';
  html += '      <!-- Seletor de período posicionado absoluto à direita -->';
  html += '      <div class="absolute right-0 top-1/2 transform -translate-y-1/2">';

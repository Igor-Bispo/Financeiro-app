...
// Assina notificaÃ§Ãµes do usuÃ¡rio atual, mantÃ©m estado em appState e exibe toasts opcionais.
...
function showWebNotification(title, opts = {}) {
  try {
    if (!isWebNotificationSupported()) return null;
    if (Notification.permission !== 'granted') return null;
    const n = new Notification(title, opts);
    // Fallback: focus app when clicked
    n.onclick = function () {
      try { window.focus(); window.location.hash = '#/notifications'; } catch {}
    };
    return n;
  } catch (e) { console.warn('showWebNotification error', e); return null; }
}
...
function isToastAllowed(prefs, notif) {
  try {
    ...
    if (!type || !budgetId) {
      return true; // nÃ£o bloqueia se faltar dados
    }
    ...
    // Default quando nÃ£o especificado: true
...
function makeToastMessage(n) {
  try {
    ...
    if (t === 'new_transaction') {
      return `Nova transaÃ§Ã£o${budget}: ${n.transactionDescricao || ''}`;
    }
    if (t === 'updated_transaction') {
      return `TransaÃ§Ã£o atualizada${budget}: ${n.transactionDescricao || ''}`;
    }
    if (t === 'deleted_transaction') {
      return `TransaÃ§Ã£o excluÃ­da${budget}: ${n.transactionDescricao || ''}`;
    }
    if (t === 'category_added') {
    ...
    if (t === 'category_deleted') {
      return `Categoria excluÃ­da${budget}: ${n.categoryNome || ''}`;
    }
    if (t === 'test_notification') {
      return `Teste de notificaÃ§Ã£o${budget}: ${n.message || ''}`;
    }
    return `Nova atividade${budget}`;
  } catch { return 'Nova atividade'; }
}

// Estado interno em window para evitar mÃºltiplas assinaturas
function getState() {
  const w = window;
  if (!w.__notifCtl) {
...
export async function startNotificationsFor(userId) {
  ...
  st.unsub = listenByRecipient(userId, (items) => {
    ...
    // Notificar UI sobre atualizaÃ§Ã£o (para badges, etc.)
    try { eventBus.emit('notifications:updated', items); } catch {}

    // Novas notificaÃ§Ãµes: toasts
    if (getToastsEnabled()) {
      try {
        for (const n of items) {
          ...
          if (!st.lastIds.has(n.id) && ts >= st.lastSeenAt) {
            ...
            try {
              // Preferir integraÃ§Ã£o por eventBus
            ...
            // TambÃ©m exibir notificaÃ§Ã£o do sistema se permitido
            (async () => {
              try {
                const allowed = await ensureWebNotificationPermission();
                if (allowed) {
                  const title = 'Financeiro â€¢ Nova notificaÃ§Ã£o';
                  const body = message;
                  const icon = '/icon-192.png';
                  showWebNotification(title, { body, icon, tag: `notif-${n.id}` });
                }
              } catch { /* ignore */ }
            })();
          }
    ...
    // Atualiza marcadores para prÃ³xima rodada
...
export async function pinOne(id, pinned = true) {
  ...
  await pin(id, pinned);
  try { eventBus.emit('snackbar:show', { message: pinned ? 'NotificaÃ§Ã£o fixada' : 'NotificaÃ§Ã£o desfixada', type: 'success', duration: 1800 }); } catch {}
}
...
export async function archiveOne(id, archived = true) {
  ...
  try { eventBus.emit('snackbar:show', { message: archived ? 'NotificaÃ§Ã£o arquivada' : 'NotificaÃ§Ã£o restaurada', type: 'info', duration: 1800 }); } catch {}
}
...
export async function archiveAllRead() {
  ...
  try { eventBus.emit('snackbar:show', { message: 'NotificaÃ§Ãµes lidas arquivadas', type: 'info', duration: 2000 }); } catch {}
}

...
export async function renderSettings() {
  ...
  try {
    ...
    const viewState = {
      ...
      pendingInvitations,
      sentInvitations,
      appVersion: 'v4.57.0', // Versão atual
      hasNewChangelog: true, // Exemplo
      lastUpdateLabel: '09/10/2025', // Data da última atualização
...
function rerenderSettingsIfVisible() {
  // Não re-renderizar se estiver no meio de uma troca de orçamento
  if (window.__changingBudget) {
    return;
  }
  
  // Usar setTimeout para permitir que a navegação aconteça primeiro
  setTimeout(() => {
    // Verificar novamente se não está trocando orçamento e se ainda está na página de settings
    if (!window.__changingBudget && window.location.hash.startsWith('#/settings')) {
      renderSettings();
    }
  }, 100);
}

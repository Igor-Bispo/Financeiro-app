...
class FinanceApp {
  ...
  toggleDarkMode() {
    ...
    this.notifications.show(
      'Modo escuro ' + (document.body.classList.contains('dark-mode') ? 'ativado' : 'desativado')
    );
  }
  ...
  getTransactionCountForCategory(categoryId) {
    ...
    return this.transactionsCache.filter((t) => t.categoryId === categoryId && t.type === 'expense')
      .length;
  }
  ...
  renderTransactions(transactions) {
    ...
    if (!transactions || transactions.length === 0) {
      container.innerHTML =
        '<p class="text-center text-secondary">Nenhuma transação encontrada</p>';
      return;
    }

    // Debug: Log das transações recebidas
    console.log(
      '🔍 Debug - Transações para renderizar:',
      transactions.map((t) => ({
        id: t.id,
        title: t.title,
  ...
  async updateSummaryCards() {
    try {
      ...
      const totalBudgetLimits = categories.reduce((total, cat) => total + (cat.limit || 0), 0);
      ...
      this.updateCardColors(
        'totalBudget',
        totalBudgetBalance,
        totalBudgetBalance >= 0 ? 'positive' : 'negative'
      );
      this.updateCardColors(
        'budgetRemaining',
        budgetRemaining,
        budgetRemaining >= 0 ? 'positive' : 'negative'
      );
  ...
  async deleteCategory(id) {
    if (
      confirm(
        'Tem certeza que deseja excluir esta categoria? Todas as transações relacionadas serão marcadas como "Sem categoria"'
      )
    ) {
      try {
  ...
  setupIntersectionObserver() {
    const animatedElements = document.querySelectorAll(
      '.transaction-item, .category-item, .summary-card'
    );
  ...
  clearUserData() {
    ...
    document.getElementById('transactionsList').innerHTML =
      '<div class="empty-state">Faça login para ver suas transações</div>';
    document.getElementById('categoriesList').innerHTML =
      '<div class="empty-state">Faça login para ver suas categorias</div>';
    document.getElementById('goalsList').innerHTML =
      '<div class="empty-state">Faça login para ver suas metas</div>';
    document.getElementById('budgetsList').innerHTML =
      '<div class="empty-state">Faça login para ver seus orçamentos</div>';
  }
...
function handleTransactionSubmit(event) {
  ...
  const transaction = {
    ...
    amount: parseFloat(
      formData.get('transactionAmount') || document.getElementById('transactionAmount').value
    ),
    type: formData.get('transactionType') || document.getElementById('transactionType').value,
    category:
      formData.get('transactionCategory') || document.getElementById('transactionCategory').value,
    date: formData.get('transactionDate') || document.getElementById('transactionDate').value,
    description:
      formData.get('transactionDescription') ||
...
function handleCategorySubmit(event) {
  ...
  const category = {
    ...
    limit:
      parseFloat(formData.get('categoryLimit') || document.getElementById('categoryLimit').value) ||
      0,
  };
...
function handleGoalSubmit(event) {
  ...
  const goal = {
    ...
    current:
      parseFloat(formData.get('goalCurrent') || document.getElementById('goalCurrent').value) || 0,
    deadline: formData.get('goalDeadline') || document.getElementById('goalDeadline').value,
    description:
      formData.get('goalDescription') || document.getElementById('goalDescription').value,
  };
...
function handleBudgetSubmit(event) {
  ...
  const budget = {
    ...
    amount: parseFloat(
      formData.get('budgetAmount') || document.getElementById('budgetAmount').value
    ),
    ...
    description:
      formData.get('budgetDescription') || document.getElementById('budgetDescription').value,
  };

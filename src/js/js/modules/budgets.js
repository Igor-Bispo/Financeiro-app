...
class BudgetManager {
  ...
  async addBudget(budgetData) {
    try {
      ...
      const members =
        Array.isArray(budgetData.members) && budgetData.members.length > 0
      ...
      if (!this.isOnline) {
        ...
        this.showNotification &&
          this.showNotification(
            'Orçamento salvo localmente. Será sincronizado quando online',
            'info'
          );
        return 'offline';
      }
  ...
  async getTotalBudget() {
    try {
      ...
      const snapshot = await this.budgetsRef.where('userId', '==', user.uid).get();
  ...
  async getBudgetsWithBalance() {
    try {
      ...
      const budgetsSnapshot = await this.budgetsRef.where('userId', '==', user.uid).get();
  ...
  async syncOfflineData() {
    try {
      ...
      this.showNotification &&
        this.showNotification('Orçamentos sincronizados com sucesso', 'success');
    } catch (error) {

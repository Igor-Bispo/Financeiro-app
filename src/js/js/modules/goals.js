...
class GoalManager {
    ...
    try {
      ...
      if (!this.isOnline) {
        ...
        this.showNotification &&
          this.showNotification('Meta salva localmente. Será sincronizada quando online', 'info');
        return 'offline';
      }
  ...
  async updateGoalProgress(id, currentAmount) {
    try {
      ...
      if (!this.isOnline) {
        ...
        this.showNotification &&
          this.showNotification(
            'Progresso salvo localmente. Será sincronizado quando online',
            'info'
          );
        return 'offline';
      }
  ...
  async clearAllGoals() {
    try {
      ...
      const snapshot = await this.goalsRef.where('userId', '==', user.uid).get();

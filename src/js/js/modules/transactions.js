...
class TransactionManager {
    ...
    try {
      ...
      if (!this.isOnline) {
        ...
        this.showNotification &&
          this.showNotification(
            'Transação salva localmente. Será sincronizada quando online',
            'info'
          );
        return 'offline';
      }
  ...
  async getTransactions() {
    try {
      ...
      if (!this.isOnline) {
        ...
        this.transactionsCache = budgetId
          ? offlineTransactions.filter((t) => t.budgetId === budgetId)
          : offlineTransactions;
        this.isLoading = false;
        return this.transactionsCache;
      }
      ...
      try {
        ...
        const budgetId = window.ActiveBudgetId;
        this.transactionsCache = budgetId
          ? fallbackData.filter((t) => t.budgetId === budgetId)
          : fallbackData;
        return this.transactionsCache;
      } catch (fallbackError) {
      ...
      if (!this.isOnline) {
        ...
        return this.transactionsCache.filter((transaction) => {
          const transactionDate = new Date(
            transaction.date || transaction.createdAt?.toDate() || 0
          );
  ...
  async clearAllTransactions() {
    try {
      ...
      const snapshot = await this.transactionsRef.where('userId', '==', user.uid).get();

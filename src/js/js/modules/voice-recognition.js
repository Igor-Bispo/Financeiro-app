...
class VoiceRecognition {
  ...
  async processTransactionCommand(transcript) {
    try {
      ...
      console.log('[VOICE][DEBUG] Reconhecido:', {
        amount,
        description,
        type,
        category,
      ...
      if (window.FinanceUI) {
        window.FinanceUI.addLog(
          `🔎 Valor: ${amount || 'não reconhecido'}, Descrição: ${description || 'não reconhecida'}, Tipo: ${type}, Categoria: ${category}`
        );
      }

      if (!amount) {
        this.showFeedback(
          'Não consegui entender o valor. Tente frases como "gastei 50 reais no mercado" ou "recebi 100".'
        );
        return;
      }
      ...
      if (window.FinanceTransactions) {
        ...
        this.showFeedback(
          `Transação adicionada: ${transaction.description} - ${window.FinanceHelpers ? window.FinanceHelpers.formatCurrency(amount) : amount}`
        );

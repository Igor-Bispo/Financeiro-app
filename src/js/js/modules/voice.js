...
class VoiceRecognition {
  ...
  updateUI() {
    ...
    if (voiceBtn) {
      ...
      voiceBtn.title = this.isListening
        ? 'Parar reconhecimento de voz'
        : 'Reconhecimento de voz (Ctrl+V)';
    }
  ...
  processVoiceCommands(transcript) {
    ...
    if (transcript.includes('nova transação') || transcript.includes('adicionar transação')) {
      ...
      transcript.includes('nova categoria') ||
      transcript.includes('adicionar categoria')
    ) {
      showCategoryModal();
    } else if (transcript.includes('nova meta') || transcript.includes('adicionar meta')) {
      showGoalModal();
    } else if (
      transcript.includes('novo orçamento') ||
      transcript.includes('adicionar orçamento')
    ) {
      showBudgetModal();
    }
  ...
  fillTransactionForm(transcript) {
    ...
    if (
      ...
      transcript.includes('ganho') ||
      transcript.includes('entrada')
    ) {
      ...
      transcript.includes('despesa') ||
      transcript.includes('gasto') ||
      transcript.includes('saída')
    ) {
      const typeField = document.getElementById('transactionType');
      if (typeField) {

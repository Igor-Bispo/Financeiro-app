...
export async function renderAnalytics() {
  ...
  try {
    ...
    if (hh !== '#/analytics') {
      console.log(`⛔ [${timestamp}] Abortando renderAnalytics: rota ativa não é /analytics (${hh})`);
      return;
    }
  } catch {}
        ...
        try {
          ...
          if (path === '#/analytics') {
            ...
            if (ym && ym !== lastYmRendered) {
              // Re-render para refletir mês selecionado
              renderAnalytics();
            }
          }
        } catch {}
  ...
  // Reagir a mudanças de período globais quando estamos na aba de análises
  try {
      ...
      eventBus.on('period:changed', (p) => {
        try {
          ...
          if (hh === '#/analytics') {
            ...
            // Registrar intenção de re-render com o novo período
            desiredYm = ym;
            if (isRenderingAnalytics) {
              rerenderRequested = true;
            } else {
              // Re-renderizar imediatamente para atualizar cards e gráficos
              renderAnalytics();
            }
          }
        } catch {}
  ...
  renderingPromise = (async () => {
    try {
      ...
      // 4. Criar HTML da página de forma síncrona (casca padrão de abas)
      ...
      // Indicador de período será montado dentro do AnalyticsPage. Evitar duplicidade de header aqui.

      // 8. Registrar ym atual e renderizar conteúdo
      try {
      ...
      // Se houve solicitação de re-render durante a renderização, e ainda estamos na rota Analytics, reaplicar
      try {
        const hh = (window.location.hash || '').split('?')[0];
        if (rerenderRequested && hh === '#/analytics') {
          rerenderRequested = false;
          // Re-render somente se o ym desejado diverge do último renderizado

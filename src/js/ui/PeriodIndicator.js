// PeriodIndicator.js - Componente reutilizável para exibir/alterar o período (mês/ano)
...
export function createPeriodIndicator(options = {}) {
  ...
  // Obter período selecionado globalmente
  const getPeriod = () => {
  ...
  const setPeriod = (y, m) => {
    if (typeof window.setSelectedPeriod === 'function') window.setSelectedPeriod(y, m);
      ...
      // Atualiza URL hash mínima (sem quebrar deep-link existente)
      try {
  ...
  if (readonly) {
    wrapper.innerHTML = `
      <div class="flex items-center gap-2 bg-white/70 dark:bg-gray-800/60 backdrop-blur rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm px-3 py-2">
        ...
        <span class="inline-flex items-center gap-1 text-xs md:text-sm px-2 py-1 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 border border-gray-200 dark:border-gray-600">📅 ${currentLabel}</span>
      </div>
    `;
    return wrapper;
  }
  ...
  wrapper.innerHTML = `
    <div class="flex flex-wrap items-center gap-2 bg-white/70 dark:bg-gray-800/60 backdrop-blur rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm px-3 py-2">
      ...
      <span class="hidden md:inline text-[11px] md:text-xs ml-1 px-2 py-1 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 border border-gray-200 dark:border-gray-600">📅 ${currentLabel}</span>
    </div>
  `;

...
vi.mock('firebase/firestore', () => {
    ...
    writeBatch: function (db) {
      ...
      const batch = {
        update(docRef, payload) {
          ops.push({ type: 'update', docRef, payload });
        },
        async commit() {
          for (const op of ops) {
            if (op.type === 'update') {
  ...
  // assign reserved name separately to avoid parser issues
...
describe('notificationsRepo', () => {
  ...
  it('listByRecipient returns items sorted by createdAt desc and limited', async () => {
    ...
    const mod = await import('@data/repositories/notificationsRepo.js');

...
// Mock the underlying repositories used by the service
...
describe('recorrentes service', () => {
  beforeEach(() => {
    // minimal appState used by the service
  ...
  it('calcularParcelaRecorrente calcula corretamente', () => {
    ...
    // dataInicio em Janeiro, referência em Março -> parcela 3
    const p = calcularParcelaRecorrente(r, 2025, 3);
    expect(p).toBe(3);
  });

  it('calcularStatusRecorrente não passa da última parcela', () => {
  ...
  it('respeita efetivarMesAtual=false (aplica só mês seguinte ao virar ano)', () => {
  ...
  it('evita duplicação quando já existe transação com mesma parcela/total no mês', async () => {
    ...
    global.window.appState.transactions = [
      {
        recorrenteId: 'rx',
        descricao: 'Plano X',
        parcelasTotal: 12,
        parcelaAtual: calcularParcelaRecorrente(rec, ano, mes),
        createdAt,
      },
    ];
  ...
  it('aplicarRecorrentesDoMes respeita forcarAplicacao=false', async () => {
    const { listByBudget } = await import('../src/data/repositories/recurringRepo.js');
    listByBudget.mockResolvedValueOnce([
      { id: 'r1', descricao: 'Netflix', ativa: true, dataInicio: '2024-01-01', parcelasRestantes: null },
    ]);
    const res = await aplicarRecorrentesDoMes('u1', 'b1', false);
    expect(res.total).toBeGreaterThanOrEqual(0);
    // When not forced, aplicadas should be 0 for newly scheduled ones
    expect(res.aplicadas).toBe(0);
  });

  it('limparLogsAntigos retorna quantidade', async () => {
    const n = await limpiarLogsAntigos('u1');
    expect(n).toBe(3);
  });
